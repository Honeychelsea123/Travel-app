...
|---|---|---|---|---|
| **일정 지도(Leaflet)** | 4317–4494 | 178 | **0** | drawPlanMap · mapLinks · memoMapUrl · splitParts · ensureLeaflet |
...
**위 넷(ctx 0~2, 713줄)이 지금 제일 싸다.** `도시 검색` 과 `일정 지도` 는
딸린 것이 아예 없다 — `loadCities`·`drawPlanMap` 을 app.js 가 많이 부르지만
(각 12곳·14곳) 그건 **내보내면 되는 것**이지 ctx 가 아니다. 방향이 반대다.
