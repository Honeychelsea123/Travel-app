...
2026-08-19. `app.js` 9,169 → 784줄. 서른일곱 조각이 나갔다. **91%를 걷었고, 여기서 멈춘다.**
...
| `notify.js` | 306 | 알림 설정 · 잠금화면 알림 · 종 | me |
...
| `swreg.js` | 110 | 서비스 워커 등록 · 새 판 확인 | logError |
...
- `추천 검사` 머리말 아래에 `parseMemo`·`nice`·`lineChips`·`drawCats`·
  ...
  그것들은 두고 왔다.
- `내 자료 내려받기` 범위 안에 `openAi`·`closeAi`·`loadNotifs` 가 있었다.
- `AI 일정 초안` 범위 안에 `showProfile` 과 상단 홈 단추 — **화면 전환 가족**이
  들어 있었다. 머리말대로 잘랐으면 초안 파일에 화면 전환이 딸려 나왔다.
  **세 번 연속 같은 함정이다**(b345 · b347 · b350).
- `일정 검토` 범위 안에 `loadAi`(AI 남은 횟수)가 있었다. 이건 떼어내면
  app.js 에 홀로 남는 20줄이라 **같이 데려갔다** — 자리가 없어서지 옳아서가
  아니다. 그런 판단은 머리말에 적어 둔다.

**자르기 전에 `선언` 을 세라**(재는 법의 3번). 머리말은 참고일 뿐이다.

### 쪼개다 보면 죽은 코드가 드러난다 (b347)

`openReview`(AI 화면에서 검토 카드를 펴 둘까)를 ctx 로 넘기려다 보니
**켜는 곳이 저장소 어디에도 없었다.** 선언(`= false`)과, 읽고 바로 끄는
두 줄이 전부다. 즉 검토 카드는 늘 접힌 채로 열린다.

`git show 0a5ec53:app.js` 로 확인해 보니 **쪼개기 전부터 그랬다.**
이번에 생긴 것이 아니다. **동작을 바꾸지 않고 그대로 옮기고 주석만 달았다** —
쪼개는 중에 기능을 같이 고치면 무엇 때문에 깨졌는지 알 수 없게 된다.

**의심스러우면 `git show <옛커밋>:app.js | grep` 으로 먼저 확인하라.**
내가 방금 깨뜨린 것인지 원래 그랬던 것인지가 갈린다.
...
## ⚠ `$` 는 서른일곱 조각 내내 검사에 **한 번도 안 보였다** (b362)

쪼개기가 끝난 뒤 실기기 점검을 하려고 `client_errors` 를 먼저 읽었더니
**오늘 날짜로 세 줄**이 있었다. 하나는 `Can't find variable: $` — WebKit
말투다. **아이폰에서 난 것이다.**

`flags.js`(b360, 서른여섯 번째)가 `$` 를 **import 없이 일곱 곳에서** 쓰고
있었다. 검사는 내내 초록불이었다.

**왜 못 봤나 — `\b` 는 `$` 앞에서 성립하지 않는다.**

```js
평평.matchAll(/\b([A-Za-z_$][\w$]*)\b/g)     // <- 이랬다
```

`\b` 는 단어글자와 아닌 것의 경계인데 `$` 는 `\w` 가 아니다. `$('noticebar')`
는 앞이 `(` 나 공백, 뒤가 `(` 라 **양쪽 다 경계가 안 선다.** 그래서 `$` 로
시작하는 이름은 통째로 안 세어졌다(끝이 `$` 인 이름도 마찬가지다).
경계 대신 **앞뒤에 이름글자가 없음**을 본다:

```js
평평.matchAll(/(?<![\w$])([A-Za-z_$][\w$]*)(?![\w$])/g)
```

**하필 제일 흔한 이름이었다.** `$` 는 dom.js 가 내보내고 거의 모든 파일이
쓴다 — 검사가 제일 자주 봐야 할 이름이 유일하게 안 보이는 이름이었다.
b335 에서 템플릿 사각지대를 막을 때와 같은 모양이다: **이름을 제일 많이
부르는 자리가 통째로 빠져 있었다.**

### 무게 — 조용한 것이 제일 나빴다

`loadFlags` 가 `(async () => {…})()` 안에서 돌아 **unhandledrejection** 으로만
남았다. 화면에는 아무 표시도 안 났다. 그런데 `drawNotice()` 가
`applyFeatures()` **앞**에 있어서, 터지면 뒤가 통째로 안 돈다:

| 안 걸린 것 | |
|---|---|
| 공지줄 | 만든 사람이 적은 안내가 안 뜬다 |
| 기능 스위치 | `push`·`docs`·`reorder` 를 꺼도 화면에 그대로 남는다 |
| **읽기전용(점검 모드)** | **`setReadOnly` 를 부르는 곳이 여기 한 곳뿐이다** |

`net.js` 의 기본값은 `readOnly = false` 다. 즉 **배포를 안 기다리고 저장을
막으려고 만든 킬스위치가 b360~b361 내내 죽어 있었다.** 켜도 아무 일도
안 일어났을 것이다. 급할 때 쓰려던 장치가 급할 때 없는 것이 제일 나쁘다.

### 확인한 방법 — 깨뜨려서 갈랐다

고친 검사만 돌려서는 "원래 조용한 검사"와 구별이 안 된다. 셋을 돌렸다:

| | 결과 |
|---|---|
| 고친 검사 + `$` 뺀 flags.js | **잡는다** (`X flags.js — import 없이 씁니다: $(dom.js)`) |
| **옛 검사 + 똑같이 깨진 flags.js** | **0개, 통과** ← 사각지대가 진짜였다는 증거 |
| 고친 검사 + `esc` 뺀 home.js | 잡는다 (다른 이름도 그대로 본다) |

가운데 줄이 없으면 "고쳐서 잡힌 것"인지 "원래 잡혔을 것"인지 모른다.
**고친 뒤에는 옛 판으로도 한 번 돌려 봐라.**

## ⚠ 세 번째 사각지대 — **아무도 안 내보내는 이름** (b364)

`$` 를 막은 그날, 사용자가 "일정 탭 `추가` 를 눌러도 아무 반응이 없다"고
했다. 눌러보니 `planSeedGeo is not defined` 였다.

`cards.js` 안에 **`let planSeedGeo = null` 로 숨어 있는데** `geocode.js` 와
`app.js` 가 그냥 이름을 부르고 있었다. 모듈은 늘 strict 라 터진다. 게다가
그 줄이 **토글보다 앞**이라 정말 아무 일도 안 일어났다 — 일정 추가가 통째로
죽어 있었다.

**검사는 또 조용했다.** 후보를 '**누군가 `export` 하는 이름**' 으로만 잡고
있었기 때문이다(잡음을 0 으로 두려던 b336 의 판단). 사문(private)은 애초에
후보가 아니었다. 맨 바깥 선언은 **내보내든 안 내보내든** 세도록 고쳤다.

같은 검사가 곧바로 **두 번째를 찾아냈다** — `aiscreen.js` 가 `cards.js` 의
사문 `lastTake` 를 건드리고 있었다(AI 대화 지우기가 거기서 터진다).
넓히지 않았으면 이것도 못 봤다.

### 후보 표에서는 넉넉히 세면 안 된다

넓히자마자 **헛짚음이 스물세 개** 났다. 전부 `null(shelf.js)` 이었다:
...
세미콜론까지 읽고 `,` 로 자르니 **괄호 안의 인자**가 선언 이름이 됐다.

`있는이름`(이 파일이 가진 것) 쪽은 넉넉히 세어도 조용해질 뿐이라 b339 주석이
"그 방향이 안전하다"고 적어둔 게 맞다. 그러나 **후보 표(`어디`)는 반대다** —
넉넉히 세면 **없는 이름을 지어낸다.** 두 표에 같은 규칙을 쓰면 안 된다.
후보 쪽은 **맨 바깥 쉼표에서만** 자르고(`바깥쉼표`), `null`·`true` 같은
값 낱말은 뺀다. 스물세 개가 0 이 됐다.

### 깨뜨려서 갈랐다 (넷)

| | 결과 |
|---|---|
| geocode 의 import 를 뺌 | 잡는다 |
| aiscreen 의 import 를 뺌 | 잡는다 |
| **사문 변수를 남의 파일이 씀** (사고 재현) | **잡는다** ← 새로 생긴 능력 |
| 양쪽이 각자 선언 | **조용하다** ← 헛짚지 않는다 |

마지막 줄이 중요하다. 같은 이름을 두 파일이 따로 두는 것은 흔한데,
그걸 말하기 시작하면 검사가 못 쓰게 된다.

### 배운 것 — 눌러보기 전에 `client_errors` 부터 읽어라

실기기 점검을 "폰을 눌러보는 일"로만 생각했는데, **오류는 이미 표에 쌓여
있었다.** `app.js` 의 `addEventListener('error')`·`unhandledrejection` 이
메시지·파일:줄·**빌드번호**·UA 까지 남긴다(보관 90일). 아무것도 안 누르고
읽기만 해서 라이브 버그를 찾았다.

보는 곳은 앱 안 **`내 계정`** 이다(대시보드가 아니다 — `errbtn` 은 버그 신고
폼 바로 아래에 있다). `복사해서 보내기` 로 20건이 글로 나온다.

⚠ **한 번 켤 때 다섯 개까지, 같은 메시지는 한 번만** 보낸다(`app.js` 의
`errSent`·`errSeen`). 여럿이 한꺼번에 나면 뒤엣것이 묻히니, 훑는 중에는
앱을 껐다 켜서 그 제한을 푼다.
