...
2026-08-19. `app.js` 9,169 → 3,403줄. 스무 조각이 나갔다. **63%를 걷었다.**
...
| 자리 | 줄 | ctx |
|---|---|---|
| 지운 것 되살리기 | 90 | 2 (`TAB_TRASH`·`loadPlans`) |
| 일정 검토 | 119 | 2 (`loadChats`·`openReview`) |
| 일정 불러오기 | 161 | 3 (`impFiles`·`loadChats`·`openAi`) |
| 오늘 화면 | 116 | 7 |
| 끌어서 순서 바꾸기 | 227 | **11** |

**끌어서 순서 바꾸기가 제일 비싸다.** 227줄인데 일정 상태를 열한 개나 만진다 —
`drawPlans` 를 먼저 정리하기 전에는 건드리지 않는 게 낫다.
싼 것부터(되살리기 · 검토 · 불러오기) 떼면 470줄이 빠지고, 그러면 남은
`여행 정보 수정`(ctx 12)과 `오늘 화면`(ctx 7)도 같이 싸진다.
