 ...
 *
 * 층: dom.js 만 씁니다. */
import { $, esc, toast, flagSprite, flagSvgOf } from './dom.js?v=b740';
/* 모험력이 서울에서의 거리를 씁니다. calc.js 는 아무것도 import 하지 않는
   잎이라 고리가 안 생깁니다. */
import { distKm, pScale, SEOUL } from './calc.js?v=b740';
...
function p16Image(code){
  return new Promise(ok => {
    ...
    /* 꼬리표를 붙입니다 — 서비스워커의 `versioned` 갈래가 **본 것만** 담고
       ...
       한 사람은 자기 유형 하나만 봅니다. */
    img.src = `./persona/${code}.webp?v=b740`;
  });
}
...
async function drawP16(s, W, H, F){
  ...
  /* ⚠⚠ **원본 비(3:2) 그대로 씁니다(b740, 사용자 지시).** ⚠⚠
   ...
   ⚠ 대신 배너 «위» 여백을 .118W → .088W 로 줄여 자(U)를 조금 돌려받습니다
     (실측 1080×1350: 남는 높이 506 → 440. 옛 1.8:1 은 506, 여백 그대로
     3:2 면 405 였습니다).
   ⚠ `drawImage` 에 잘라내기 좌표를 안 넘깁니다 — 통째로 그립니다. */
  const bw = R - L, bh = bw / 1.5, by0 = W * .088;
  {
    g.save();
