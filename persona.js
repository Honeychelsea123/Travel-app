...
import { $, esc, toast, copyText } from './dom.js?v=b408';
import { sb } from './db.js?v=b408';
import { cities, countryName, continentOf } from './cities.js?v=b408';
/* 닮은 도시로 다음 갈 곳을 고릅니다. **AI 를 안 씁니다** — 오프라인에서도
   돌아야 하고 같은 자료에는 늘 같은 답이 나와야 합니다(rec.js 맨 위 참고). */
import { similarPicks } from './rec.js?v=b408';
/* 친구와 궁합. 유입이 유입을 만드는 고리입니다(b408) — mate.js 머리말 참고. */
import { mateCode, mateLink, mateHtml } from './mate.js?v=b408';
import { visited } from './rate.js?v=b408';
import { personaStats, personaAxes, personaRank, personaMates, personaMrz,
         PERSONA16, AXIS_WORD, AXIS_NAME,
         shareCard, cardImage } from './card.js?v=b408';
...
async function drawPersona(s, ax, rates){
  ...
  $('personabox').innerHTML = `
    ...
    : `<div style="margin-bottom:var(--s-sm)">
      ...
      <button class="ghost" id="p_mate"
              style="width:100%; margin-top:8px">친구와 궁합 보기</button>
    </div>`}
