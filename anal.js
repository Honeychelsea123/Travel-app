...
import { cities, continentOf } from './cities.js?v=b521';
/* personaBackTo 는 persona.js 것입니다 — 「분석에서 왔다」를 적어두면
   닫을 때 분석 탭으로 돌아옵니다(b453). */
import { personaBackTo } from './persona.js?v=b521';
import { personaAxes, personaRank, PERSONA16,
         AXIS_NAME, AXIS_WORD } from './card.js?v=b521';
import { UN_COUNTRIES, CONT, CONT_VIEW, mapBackTo } from './map.js?v=b521';
/* 손가락으로 돌려 보는 지구본(b519) — 발자국 카드의 지도가 이것입니다. */
import { mountGlobe } from './globe.js?v=b521';
...
import { pickCity } from './citysearch.js?v=b521';
...
export async function loadAnal(){
  ...
  if (매긴것.length >= 문턱){
    ...
    머리.innerHTML = `<div class="pmeta"><div class="pcode">${esc(ax.code)}</div>
      ...
      <div class="part"><img src="./persona/${esc(ax.code)}.png?v=b521"
