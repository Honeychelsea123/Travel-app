...
import { cities, continentOf } from './cities.js?v=b495';
/* personaBackTo 는 persona.js 것입니다 — 「분석에서 왔다」를 적어두면
   닫을 때 분석 탭으로 돌아옵니다(b453). */
import { personaBackTo } from './persona.js?v=b495';
import { personaAxes, personaRank, personaMates, PERSONA16, AXIS_WORD,
         AXIS_NAME, checkList, shareCard } from './card.js?v=b495';
import { UN_COUNTRIES, CONT, mapBackTo, funRows } from './map.js?v=b495';
...
import { checkUrl } from './check.js?v=b495';
import { similarPicks } from './rec.js?v=b495';
/* 여행 만들기로 바로 잇습니다(b463) — newtrip.js 는 anal.js 를 모르므로
   고리가 안 생깁니다(확인함). */
import { openNew } from './newtrip.js?v=b495';
import { pickCity } from './citysearch.js?v=b495';
import { shareMate } from './mate.js?v=b495';
...
export async function loadAnal(){
  ...
  if (매긴것.length >= 문턱){
    ...
    머리.innerHTML = `<div class="pmeta"><div class="pcode">${esc(ax.code)}</div>
      ...
      <div class="part"><img src="./persona/${esc(ax.code)}.png?v=b495"
