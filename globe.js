...
export function mountGlobe(canvas, 갔다, 처음경도, 처음위도){
   ...
   *   잘 안 먹는다」가 그것이었습니다.
   *
   *   **그 전제가 b542 에 없어졌습니다.** 지구본이 살던 분석 탭은 카드가
   *   넷이라 길게 굴러가는 화면이었는데, 지금 사는 기록 탭은 **카드 하나
   *   짜리 한 화면**입니다(지구본 240 + 숫자 60 + 넘김 111 + 띠 60).
   *   굴릴 것이 없으면 굴리기를 뺏을 일도 없습니다.
   *
   * ⚠⚠ **그러니 기록 탭에 카드를 더 붙이려거든 여기부터 보십시오.** ⚠⚠
   *   화면이 한 장을 넘기는 순간 b535 가 그대로 돌아옵니다. 그때는
   *   `app.css` 의 `.globebox canvas` 를 `pan-y` 로 되돌리고 아래
   *   방향 잠금에서 세로를 다시 브라우저에 넘기십시오 — 고칠 곳은 두
   *   군데뿐이고, 둘 다 이 주석이 가리키고 있습니다.
