 ...
 *   돌리면 **빈 구멍**이 보입니다. 그래서 «남»으로 기울이는 것을 52° 로
 *   막습니다 — 그 안에서는 구멍이 지평선 너머에 있습니다. 이 값을
 *   늘리려거든 남극 좌표부터 넣으십시오.
...
import { 가진땅, 나라셀, 도시있나, 상자자르기 } from './citymap.js?v=b713';
import { countryName } from './cities.js?v=b713';
import { visited, myRates } from './rate.js?v=b713';

/* 화면에 있는 경로를 한 번만 읽어 경위도로 바꿔 둡니다. 돌릴 때마다 다시
   파싱하면 손가락을 따라올 수 없습니다(점이 만 개입니다). */
let 나라들 = null;

/* ── 경로 하나를 고리들로 (b696 에 `읽기` 에서 떼어냈습니다) ───────────
   ⚠ **50m 나라 파일(map50/XX.js)도 «같은 문법·같은 좌표계»입니다.** 그래서
     이 함수 하나로 둘 다 읽습니다 — 나라 화면에 들어갈 때 그 나라만 자세한
     윤곽으로 바꿔 그립니다(`자세히`). */
function 고리읽기(d){
  {
    const 고리 = [];
...
function 읽기(){
  ...
  for (const p of g.querySelectorAll('path')){
    const 고리 = 고리읽기(p.getAttribute('d') || '');
...
/* ── 기울기 한계 ── **위와 아래가 다릅니다(b710, 사용자 요청)** ─────────
 * 사용자: 「러시아는 북쪽 부분이 안 보이고 북극까지도 보고 싶어」.
 * ⚠ ±52 로 «똑같이» 막았던 것은 머리말의 남극 구멍 때문이었습니다.
 *   그런데 **구멍은 남쪽에만 있습니다.** 북쪽은 자료가 +83 까지 있고
 *   그 위는 진짜로 바다(북극해)라 비어 있는 것이 맞는 그림입니다.
 * ⚠ 게다가 북으로 기울일수록 남극 구멍은 **더 확실히 숨습니다** —
 *   정사도법의 지평선이 위도 φ0−90 이므로 φ0=+85 면 −5° 아래가 전부
 *   뒤통수입니다. 곧 북쪽을 여는 것이 구멍을 «드러내지» 않습니다.
 * ⚠ 확대했을 때가 진짜 이유입니다. 4배에서는 한 화면이 20° 남짓이라
 *   가운데를 52°N 위로 못 올리면 **시베리아 북쪽에는 아예 못 갑니다.**
 * ⚠ 남쪽 52 는 그대로입니다. 여기를 열려거든 남극 좌표부터 넣으십시오. */
const 북끝 = 85 * RAD, 남끝 = 52 * RAD;
const 기울제한 = φ => Math.max(-남끝, Math.min(북끝, φ));
...
/* ── 종이 결(b669, 사용자 결정: 「5번만 하자」) ──────────────────────
 * 바다에만 아주 옅은 결을 깝니다. 크림색이 «색»이 아니라 **종이**가
 * 됩니다. 시안에서 재보니 평균 차이가 2 정도로 «거의 안 보이는» 값인데,
 * 그게 맞습니다 — 보이기 시작하면 얼룩입니다.
 * ⚠⚠ **판을 한 번만 만듭니다.** 매 프레임 난수를 새로 뿌리면 결이
 *   «지지직»거려서 종이가 아니라 노이즈가 됩니다. 게다가 96×96 이미지를
 *   초당 30번 만들 이유가 없습니다.
 * ⚠ 그리는 자리는 **바다 위·땅 아래**입니다. 땅과 다녀온 오렌지 위에
 *   깔면 도장색이 탁해집니다.
 * ⚠ 0.02 입니다. 올리고 싶어지면 **어두운 방에서 OLED 로** 먼저
 *   보십시오 — 밝은 데서 안 보이는 값이 거기서는 얼룩으로 보입니다. */
let 결판 = null;
function 종이결(){
  if (결판) return 결판;
...
export function mountGlobe(canvas, 갔다, 처음경도, 처음위도, 누름){
   ...
   * ⚠⚠ **b711 에 20 → 40 으로 더 열었습니다**(사용자 요청: 「줌을 더
   *   들어갈 순 없어?」). 값이 아니라 «무엇이 한계인가»가 바뀌지
   *   않았음을 적어 둡니다 —
   *     · 50m 선분 하나가 20배에서 4.3px, 30배 6.4px, **40배 8.6px** 입니다.
   *       곧 40배에서는 «해안선»이 다각형으로 읽힙니다. 그 아래 자료가
   *       없어서지 그리는 값이 모자라서가 아닙니다.
   *     · 다만 그만큼 들어가면 화면에 해안이 안 걸리는 일이 많습니다 —
   *       나라 «안»이면 보이는 것은 도시 칸(원래 곧은 선)과 이름뿐이라
   *       각져 보일 것이 없습니다. 각이 보이는 것은 섬·해안 도시입니다.
   *     · 40배면 한 화면이 약 3.2° — 서울에서 대전까지가 꽉 찹니다.
   *   더 열려거든 **자료부터**입니다(10m 축척). 값만 올리면 다각형입니다. */
  const 배율끝 = [1, 40];
  ...
  const 그리기 = () => {
    ...
    /* ⚠⚠ **모서리는 지구가 아닙니다(b667, 사용자 신고: 「빨간 동그라미
       친 영역도 아래로 스크롤하면 지구본이 돌아가고 화면이 안 내려가」).**
       칸은 네모인데 지구는 동그라미라 **네 모서리는 빈 자리**입니다.
       그런데 캔버스가 거기까지 깔려 있고 `touch-action:none` 이라
       (그건 지구를 위아래로 기울이려고 준 것입니다 — 위 app.css 주석),
       모서리에 손가락을 대면 **화면이 안 굴러가고 지구만 돌았습니다.**
       b666 에 지구본을 키우면서 이 «죽은 자리»도 같이 커졌습니다.
     → `clip-path` 로 동그라미 밖을 잘라냅니다. 잘린 데는 그림도 안
       그려지고 **누름도 안 받습니다** — 손가락이 그대로 판으로 내려가
       화면이 굴러갑니다. 이 한 줄이 b594 계열 문제의 뿌리를 막습니다.
     ⚠⚠ **b668 에 «지구 딱 그 크기»로 좁혔습니다**(사용자: 「지구본에서
       조금이라도 나가면 아래로 스크롤되게」). b667 은 R×1.10 으로
       잘랐는데, 그 20px 남짓한 테는 **그림이 거의 안 보이면서 손가락만
       막고 있었습니다** — 대기 자국이 거기 그려져 있었기 때문입니다.
       그 자국을 **캔버스 밖(부모 배경)으로 옮겨서** 이 테를 열었습니다.
       부모(`.globebox`)는 `touch-action` 이 예사값이라 화면이 굴러갑니다.
     ⚠ `R + 1` 인 것은 지구 테두리 선(lineWidth 0.9)의 «바깥 절반»을
       살리기 위해서입니다. 딱 R 로 자르면 그 머리카락 선이 반쯤 깎여
       가장자리가 지저분해집니다.
     ⚠ 같은 값이면 다시 안 넣습니다 — 매 프레임 style 을 건드리면
       그때마다 배치를 다시 계산합니다. */
    {
      const 자를것 = `circle(${(R + 1).toFixed(1)}px at ${
                      cx.toFixed(1)}px ${cy.toFixed(1)}px)`;
      if (canvas.style.clipPath !== 자를것) canvas.style.clipPath = 자를것;
      /* ⚠⚠ **대기 자국은 이제 «부모의 배경»입니다(b668).** 캔버스에
         그리면 그 자리가 손가락을 막습니다. 배경은 «칠»이라 누름을
         안 받으므로, 똑같이 보이면서 화면은 굴러갑니다.
       ⚠ 아래 캔버스 가운데 값과 **같은 수를 씁니다** — 어긋나면 자국이
         ...
         지구 전체에 파란 안개가 껴서 다녀온 나라의 파랑이 죽습니다. */
    {
      ...
      /* ⚠⚠ **파란 대기광을 걷었습니다(b585).** 빛 번짐은 «유리»의 문법입니다 —
         ...
         ⚠ **번지는 폭(1.10)은 그대로 둡니다.** 위 R 계산의 1.11 과 «짝»이라
           ...
           걸렸습니다). 색만 바꿉니다. */
      /* ⚠⚠ **b668 에 여기서 «부모의 배경»으로 옮겼습니다.** 위
         `clipPath` 옆 주석에 이유가 있습니다 — 이 자국이 캔버스에
         있으면 지구 밖 20px 테가 손가락을 막습니다. 값(0.94·1.10·
         .055·.022)은 **그대로 옮겼습니다** — 보이는 것은 같습니다.
       ⚠ 되돌리려거든 «둘 다» 되돌리십시오. 여기만 되살리면 자국이
         두 겹이 되어 지구 둘레가 탁해집니다. */
    }
    ...
    /* ── ②½ 종이 결(b669) ── 바다 «위», 땅 «아래» ────────────────────
       머리말의 `종이결` 주석에 왜 이 자리인지 적어 뒀습니다. */
    {
      const 무늬 = ctx.createPattern(종이결(), 'repeat');
      if (무늬){
        ctx.save();
    ...
    /* ⚠ **확대하면 나라 칠이 옅어집니다(b707, 사용자 결정: 「나라칠은 옅게
       ...
       ⚠⚠ **0.62 → 0.90 으로 더 걷었습니다(b713).** ⚠⚠
         사용자 신고: 「안 간 도시가 색이 안 옅어지고 계속 똑같은데?」
         **칸 색이 아니라 이 칠이 문제였습니다.** 0.62 면 확대해도 0.38 이
         남는데, 안 간 도시 칸이 그 위에 얹는 것은 0.03 입니다 — 곧 보이는
         것의 거의 전부가 이 바탕이었고, 칸 값을 0.13→0.06→0.03 으로
         내려도 눈에는 그대로였습니다(b711·b712 두 판을 헛썼습니다).
         이제 0.10 만 남습니다. 겹쳐서 재면
           · 안 간 도시  1-(1-0.10)(1-0.03) = **0.13**
           · 다녀온 도시 1-(1-0.10)(1-0.62) = **0.66**  → 다섯 배 차이.
    ...
    if (배율 > 도시시작){
      ...
      /* ⚠⚠ **가로와 세로를 «따로» 잽니다(b710).** ⚠⚠
       * 처음에는 하나(`창반`)로 잡았는데 그것이 **경도 쪽에서 모자랐습니다.**
       * 지도 단위 1 은 세로로는 늘 같은 거리지만 «가로»로는 위도가 높을수록
       * 짧습니다 — 화면 1px 을 채우는 데 위도 60°에서는 지도 단위가 두 배
       * 필요합니다(1/cos60 = 2).
       * 그래서 유럽·러시아처럼 높은 데서 확대하면 셀과 해안 조각이 화면
       * «안»에서 **곧은 세로선으로 뚝 잘렸습니다** — 사용자 신고
       * 「색칠도 레이아웃이 무너질 때가 있어」가 이것입니다. 조각이 통째로
       * 창 밖으로 판정되면 자를 길이 없어 그 나라가 아예 사라지기도 했습니다.
       * ⚠ 극에서는 모든 경도가 한 화면에 들어오므로 500(지도의 절반)에서
       *   멈춥니다 — 그 위로 늘려 봐야 같은 땅을 두 번 그릴 뿐입니다. */
      const 단위px = R * Math.PI / 500;
      const 창반y = (h / 2) / 단위px + 6;
      const 창반x = Math.min(500,
        (w / 2) / 단위px / Math.max(0.05, Math.cos(φ0)) + 6);
      const 가운데x = (λ0 / RAD + 180) / 360 * 1000;
      const 가운데y = (90 - φ0 / RAD) / 180 * 500;
      const 창 = [가운데x - 창반x, 가운데y - 창반y,
                  가운데x + 창반x, 가운데y + 창반y];

      for (const 나라 of 목록){
        if (!나라.code || !도시있나(나라.code)) continue;
        const { 도시, 셀, 걸침 } = 나라셀(나라.code);
        if (!도시.length) continue;
        /* ── 화면에 안 걸린 나라를 거릅니다 ──────────────────────────
         * ⚠⚠ **b711 에 «나라 한가운데 점»에서 이 상자로 바꿨습니다.** ⚠⚠
         *   전에는 나라의 한가운데(`나라.핀`)가 화면에서 얼마나 떨어졌나로
         *   쟀습니다. 그래서 **큰 나라가 통째로 사라졌습니다** — 모스크바를
         *   들여다보면 러시아의 한가운데는 시베리아라 6배쯤부터 걸러졌고,
         *   그 나라 도시가 하나도 안 나왔습니다.
         *   `걸침` 은 그 나라 도시 상자에 21°를 더한 테두리입니다 —
         *   왜 «자르는 상자» 를 안 쓰는지는 citymap.js 에 적어 뒀습니다.
         * ⚠ 자료 받기(`가진땅`)를 **이 뒤로** 옮겼습니다 — 화면에 걸린
         *   나라만 50m 를 받습니다. 거르는 이유의 절반이 그것입니다.
         * ⚠⚠ **날짜변경선(b712).** ⚠⚠ 지도 단위 x 는 0 에서 1000 까지
         *   «끊겨» 있는데 지구는 안 끊겨 있습니다. 그래서 179°W 쯤을
         *   들여다보면 창은 x≈0 근처인데 피지는 x≈994 라, 이어져 보이는
         *   두 자리가 상자 셈에서는 지구 반대편이 됩니다 — 확대하면
         *   그 나라 도시 칸이 통째로 안 나왔습니다.
         *   ⚠ **고치는 자리는 «창»입니다, 나라가 아닙니다.** 나라 좌표를
         *     옮기면 셀·해안·도시 자리를 다 옮겨야 합니다. 창을 ±1000
         *     옮기면 비교 한 줄로 끝납니다.
         *   ⚠ **그리는 쪽은 손댈 것이 없습니다.** `던져` 는 x 를 경도로
         *     바꿔 `sin(λ-λ0)`·`cos(λ-λ0)` 에만 쓰는데, x 를 1000 옮기는
         *     것은 경도를 360° 옮기는 것이라 **같은 점**입니다.
         *   ⚠ 아래로는 `창` 이 아니라 이 `창c` 를 씁니다 — 조각 거르기와
         *     `상자자르기` 둘 다입니다. 하나만 옮기면 어긋납니다. */
        const 옮김 = 걸침[0] > 창[2] ? 1000 : 걸침[2] < 창[0] ? -1000 : 0;
        const 창c = [창[0] + 옮김, 창[1], 창[2] + 옮김, 창[3]];
     ...
     * ⚠⚠ **미는 동안에도 그립니다(b710 에 되돌림).** ⚠⚠
     *   b707 에는 「글자가 떨린다」며 끌 때·미끄러질 때 건너뛰었습니다.
     *   그랬더니 **이름이 나왔다 사라졌다** 했습니다 — 사용자 신고:
     *   「도시 이름이 보였다 안 보였다 간헐적으로」. 손대고 있는 내내
     *   이름이 없다가 멈추면 툭 나타나니 고장으로 읽힙니다.
     *   이름이 지도와 «같이» 움직이는 것은 떨림이 아닙니다(구글 지도도
     *   그렇습니다). 값도 쌉니다 — 실측 130개 0.82ms.
     * ⚠ 자리가 겹치면 버립니다 — 큰 것부터가 아니라 «먼저 온 것»이 이깁니다.
     *   도시를 먼저 두는 이유가 그것입니다(확대했으면 도시가 주인공).
     * ⚠ **넘어가는 자리에서 «톡» 꺼지지 않게 겹쳐서 흐립니다(b710).**
     *   전에는 도시 이름이 섞 0.55 에서, 나라 이름이 0.9 에서 한 프레임에
     *   생겼다 없어졌습니다. 이제 둘 다 0.3 폭으로 나타나고 사라집니다 —
     *   그 사이에는 둘이 같이 옅게 보입니다. */
    if (배율 > 1.5){
      const 도시짙기 = Math.min(1, Math.max(0, (섞 - 0.35) / 0.3));
      const 나라짙기 = Math.min(1, Math.max(0, (0.95 - 섞) / 0.3));
      const 놓은 = [];
      const 쓰기 = (글, x, y, 크기, 진하기, 짙기) => {
        if (!글 || 짙기 <= 0.02) return;
      ...
      for (const 나라 of 목록){
        ...
        쓰기(countryName[나라.code] || '',
             ...
             cy - R * (cφ0 * sφ - sφ0 * cφ * cΔ), 12, '500', 나라짙기);
      }
    }
  };
  ...
  let 굴림 = null;
  /* ⚠ **「전체」로 돌아오면 다시 천천히 돕니다(b666, 사용자 요청:
     「전체 부분에서는 천천히 지구가 돌았으면 좋겠는데 안도네?」).**
     그동안 안 돈 이유: 넘김이 첫 자리를 잡을 때 `지도맞추기` 가
     `처음으로()` 를 부르고, 그것이 `회전()` → `세우기()` 로 이어져
     **열자마자 자동 회전이 꺼졌습니다.** 자기가 자기를 껐던 것입니다.
   ⚠ 대륙 하나를 고른 것은 «거기를 보겠다»는 뜻이라 그때는 안 돕니다.
     도는 것은 「전체」에서만 — 그래서 `처음으로()` 만 이 표를 답니다.
   ⚠ 굴러가는 «도중»에 켜면 안 됩니다. 굴림이 끝난 뒤에 이어받아야
     대한민국에 닿고 나서 천천히 흐릅니다. */
  let 자동뒤에 = false;
  /* 나라 화면에 들어가기 «전»의 자리. 나올 때 여기로 돌아옵니다(b693). */
  let 되돌 = null;
  /* ── 한 나라만 «자세히»(b696) ────────────────────────────────────────
   * 지구본은 110m 이라 한국이 점 19개짜리 윤곽입니다. 그 위로 당기면 각진
   * 덩어리가 되어서 b695 는 8배에서 멈춰야 했습니다.
   * → 나라 화면에 들어갈 때 **그 나라만** 50m 윤곽으로 바꿔 그립니다.
   *   자료는 어차피 그때 받고 있으니 «공짜»입니다(한국 2.2KB).
   * ⚠ 매 프레임 도는 점이라 **4,000점까지만** 받습니다. 그보다 큰 나라
   *   (캐나다·러시아·미국)는 어차피 배율이 1~2배라 자세할 필요가 없습니다.
   * ⚠ 하나만 들고 있습니다 — 나라를 옮기면 옛 것은 버립니다. */
  let 자세한 = null;                 /* { code, 고리 } */
  /* 처음 빠르고 끝에서 느리게. 뚝 서지 않아야 「도착했다」로 읽힙니다. */
  const 눅임 = t => 1 - Math.pow(1 - t, 3);
  /* ── 얼마나 걸릴 것인가 (b701) ───────────────────────────────────────
   * 사용자: 「어떤 나라들은 너무 빨라서 어디로 줌인되는지도 모르겠어.」
   * ⚠ **고정 0.62초가 문제였습니다.** 한국·영국은 1배에서 14배까지 가는데
   *   러시아는 1 → 1.3 입니다. 같은 시간을 주면 앞엣것은 순간이동입니다.
   * ⚠ 돌아가는 각도와 «배율이 몇 곱절 되는가»를 같이 봅니다. 배율은 비율이
   *   눈에 보이는 양이므로 로그로 셉니다(1→14 와 1→1.3 은 2.6 대 0.26).
   * ⚠ **줄어드는 쪽은 절반만 셉니다.** 확대해 둔 채로 큰 나라를 누르면
   *   「들어가는데 오히려 멀어지는」 그림이라, 길게 끌면 더 어색합니다.
   * ⚠ 0.55~1.25 초로 자릅니다. 그보다 짧으면 못 따라가고, 길면 답답합니다. */
  const 걸릴시간 = (dλ, dφ, 배0, 배1) => {
    const 돌 = Math.hypot(dλ || 0, dφ || 0);              /* 라디안 */
    const 줌 = 배1 > 배0 ? Math.log(배1 / 배0) : Math.log(배0 / 배1) * 0.5;
    return Math.max(0.55, Math.min(1.25, 0.55 + 0.26 * 돌 + 0.20 * 줌));
  };
  /* 「여기로 데려다 달라」는 부탁(다가가기·앉히기)이 넘지 못할 선입니다.
   ⚠ **b711 부터 손가락과 «같은» 값입니다.** 예전에는 14 로 따로 뒀습니다 —
     손가락이 4배까지였고 나라 화면으로 들어갈 때만 그 위로 갔기 때문입니다.
     나라 화면이 없어졌고(b707) 손가락도 40배까지 가므로 두 벌로 둘 이유가
     없어졌습니다. 두 벌이면 언젠가 갈라집니다. */
  const 최대배 = () => 배율끝[1];
  /* ⚠⚠ **덮여 있으면 돌지 않습니다(b688, 사용자 신고: 「앱이 무겁고 렉도
     걸리고」).** `보임` 은 IntersectionObserver 가 정하는데, 그것은 «창 안에
     있나»만 봅니다 — 위에 무엇이 덮였는지는 **모릅니다.** 나라 지도
     (#cmappane)는 position:fixed 로 화면을 다 덮을 뿐 지구본을 안 지워서,
     그동안에도 지구가 30fps 로 9,918개 점을 계속 다시 투영하고 있었습니다.
     (도시 화면은 #tabdeck 에 hide 를 걸어 관찰자가 알아채므로 멀쩡했습니다.)
   ⚠ 덮은 쪽이 «알려 줘야» 합니다 — 덮였는지를 여기서 알아낼 방법이 없습니다.
     `덮임(true/false)` 을 내보냅니다. */
  let 가려짐 = false;
  const 돌까 = () => (자동 || 관성 || 굴림) && 보임 && !가려짐;
  function 한바퀴(t){
    루프 = 0;
    ...
    if (굴림){
      ...
      λ0 = 굴림.λ + 굴림.dλ * k;
      φ0 = 굴림.φ + 굴림.dφ * k;
      /* 굴러가면서 «다가가기»도 합니다(b693) — 나라를 누르면 그 나라로
         돌면서 확대되고, 그 위로 나라 화면이 겹쳐 뜹니다. */
      if (굴림.d배) 배율 = 굴림.배0 + 굴림.d배 * k;
      if (굴림.간 >= 굴림.동안){
        굴림 = null;
        if (자동뒤에){ 자동뒤에 = false; 자동 = true; }
      }
    } else if (관성){
      λ0 += 관성 * dt;
      관성 *= Math.pow(0.06, dt);
  ...
  /* ⚠ `자동뒤에` 도 **같이 끕니다.** 굴러가는 도중에 손이 닿았는데
     표가 남아 있으면, 손을 뗀 뒤에 제멋대로 다시 돌기 시작합니다. */
  const 세우기 = () => { 자동 = false; 관성 = 0; 굴림 = null; 자동뒤에 = false; };
  ...
  canvas.addEventListener('pointermove', e => {
    ...
    /* ⚠ 6px 은 손이 떨리는 폭입니다. 그 전에는 아직 아무것도 안 합니다 —
     ...
     ⚠⚠ **전에는 이 문턱이 «손가락에만» 있었습니다(b698에 고침).** ⚠⚠
       `끌기.손가락` 은 `pointerType !== 'mouse'` 라, 마우스로 누르면
       0픽셀짜리 `pointermove` 하나만 와도 그대로 아래로 내려가
       **`민적 = true`** 가 됩니다. 그러면 손을 뗄 때 「민 것」으로 쳐서
       **나라를 눌러도 아무 일도 안 일어납니다**(`놓기` 의 `!민적` 문).
       ⚠ 실제로 재현했습니다 — 크롬에서 일본을 눌렀을 때 온 순서가
         `pointerdown → pointermove(같은 자리) → pointerup` 이었고,
         나라 화면이 안 떴고 지구도 안 돌았습니다(dx=dy=0 이라).
       ⚠ 폰에서 안 걸린 이유가 이것입니다 — 손가락은 문턱을 탔습니다.
         **노트북·태블릿 마우스에서만** 나던 고장입니다.
       ⚠ 마우스는 손보다 덜 떨리니 문턱을 3px 로 둡니다. */
    if (!끌기.깨움){
      const 문턱 = 끌기.손가락 ? 6 : 3;
      if (Math.abs(dx) < 문턱 && Math.abs(dy) < 문턱) return;
      끌기.깨움 = true;
    }
  ...
  return {
    ...
    /* ⚠ 확대도 같이 풉니다 — 「전체」로 돌아왔는데 지구가 확대돼 있으면
       처음 화면이 아닙니다. */
    /* ⚠ 표는 `회전` **뒤에** 답니다 — `회전` 이 `세우기()` 로 시작해서
       먼저 달면 그 자리에서 지워집니다(b666). */
    처음으로(){ 배율 = 1; this.회전(KR[0], KR[1]); 자동뒤에 = true; },
    /* ── 그 나라로 «다가가기»(b693, 사용자 요청) ────────────────────────
     * 사용자: 「지구본에서 국가 누르면 그냥 국가페이지로 뚝 끊겨서 카드가
     *   뜨는데, 지구본이 그 국가로 줌 되는 것처럼 하면서 국가페이지가 뜨는
     *   애니메이션 효과도 가능해?」
     * ⚠ 굴리기와 «같은 굴림»에 태웁니다. 따로 돌리면 둘의 눅임이 어긋나
     *   확대가 회전보다 먼저 끝나거나 늦게 끝납니다.
     * ⚠ 돌아올 자리를 적어 둡니다 — 화면을 닫으면 보던 자리로 되돌아가야
     *   「들어갔다 나왔다」로 읽힙니다. */
    /* ⚠ **손가락 한계(4배)보다 더 당깁니다(b695, 사용자 요청: 「확대배율을
       더 크게, 실제 대륙이랑 비슷한 사이즈까지 가서 전환」).** 손으로 돌릴
       때 4배가 한계인 것은 110m 지도가 그 위로는 각져 보이기 때문인데,
       들어가는 0.6초 동안은 «곧 덮일 그림»이라 그 값이 다릅니다.
       ⚠ 나온 뒤에는 `되돌리기` 가 원래 배율로 돌려놓습니다. */
    /* ⚠ 시간을 «안 받으면» 이동 거리에서 스스로 냅니다(b701, 위 `걸릴시간`).
       ⚠ **얼마나 걸리는지 돌려줍니다** — 부르는 쪽(ctrymap)이 그 시간에
         맞춰 판을 띄우고 지구본을 덮어야 합니다. 전에는 560ms 로 못 박혀
         있어서, 굴림(620ms)이 끝나기 «전»에 지구본이 얼어붙었습니다. */
    다가가기(경도, 위도, 목표 = 2.6, 동안 = 0){
      되돌 = { λ: λ0, φ: φ0, 배: 배율 };
      this.회전(경도, 위도);              /* 굴림을 만듭니다 */
      const 끝배 = Math.max(1, Math.min(최대배(), 목표));
      굴림.배0 = 배율;
      굴림.d배 = 끝배 - 배율;
      굴림.동안 = 동안 || 걸릴시간(굴림.dλ, 굴림.dφ, 배율, 끝배);
      return 굴림.동안;
    },
    /* 그 나라를 자세한 윤곽으로 그립니다. `d` 는 map50 의 경로 문자열입니다.
       돌려주는 값은 «받은 점 수»(0 이면 안 씁니다). */
    자세히(code, d){
      자세한 = null;
      if (!code || !d) return 0;
      const 고리 = 고리읽기(String(d));
      let n = 0; for (const r of 고리) n += r.length;
      if (!n || n > 4000) return 0;      /* 매 프레임 돌기엔 너무 많습니다 */
      자세한 = { code, 고리 };
      다시();
      return n;
    },
    /* 보던 자리로. 적어 둔 것이 없으면 아무 일도 안 합니다. */
    되돌리기(동안 = 0){
      자세한 = null;                     /* 나오면 다시 110m 으로 */
      if (!되돌) return 0;
      const 그 = 되돌; 되돌 = null;
      this.회전(그.λ / RAD, 그.φ / RAD);
      굴림.배0 = 배율;
      굴림.d배 = 그.배 - 배율;
      /* 나가는 길도 들어온 길과 같은 자로 잽니다(b701) — 다만 나가는 것은
         조금 빠른 편이 시원합니다(0.85배). */
      굴림.동안 = 동안 || 걸릴시간(굴림.dλ, 굴림.dφ, 배율, 그.배) * 0.85;
      return 굴림.동안;
    },
    ...
    /* ⚠⚠ **덮개를 걷을 때 «보임» 을 손으로 켭니다(b701).** ⚠⚠
       `보임` 은 IntersectionObserver 가 정하는데 그 콜백은 **다음 프레임에나**
       옵니다. 덱을 방금 `display:none` 에서 되살린 참이라 이 순간 `보임` 은
       아직 거짓이고, 그래서 바로 뒤따라오는 `되돌리기` 가 굴림을 만들어도
       `깨우기` 의 `돌까()` 가 거짓이라 **한 프레임도 안 굴렀습니다.**
       판이 사라진 뒤 확대된 화면이 얼어붙어 있다가 뒤늦게 줌아웃이
       시작되던 것이 이것입니다(사용자 지적: 「처음으로 돌아갔다가 다시
       확대된 화면에서 줌 빠지는 게 이상해」).
     ⚠ `다시()` 가 아니라 `그리기()` 를 «바로» 부릅니다 — `다시` 는 rAF 라
       숨은 창에서 안 옵니다. 덮개가 걷힌 그 순간의 그림이 맞아야 합니다.
     ⚠⚠ **`clientWidth` 를 보고 켭니다(b702).** 그냥 켜면 «2D 보기»에서
       탈이 납니다 — 그때는 지구본 칸이 `display:none` 이라 폭이 0 인데,
       보임을 참으로 만들면 b688 이 막아 둔 「안 보이는데 계속 그리기」가
       그대로 돌아옵니다. 폭이 있을 때만 켭니다. */
    덮임: (b) => {
      가려짐 = !!b;
      if (!가려짐){
        if (canvas.clientWidth) 보임 = true;
        마지막 = 0; 그리기(); 깨우기();
      }
    },
    끝: () => {
    ...
    /* ── 나라 지도와 그림을 «주고받는» 창구(b702) ──────────────────────
     * 나라 화면은 평면(등장방형)이고 지구본은 정사도법인데, b699 에서 나라
     * 지도에 `cos(가운데위도)` 를 곱한 뒤로 **두 그림이 국소적으로 거의
     * 같아졌습니다**(반각 6° 에서 0.55% 차이). 그래서 «지금 지구본이 그리고
     * 있는 자리»를 그대로 나라 지도의 viewBox 로 옮겨 적을 수 있습니다.
     * 그 덕에 디졸브 없이 이어 붙일 수 있습니다.
     * ⚠ `판` 은 «마지막으로 그린» 값입니다. 한 번도 안 그렸으면 null 이니
     *   부르는 쪽은 **null 이면 이음을 건너뛰어야** 합니다. */
    상태(){
      if (!판) return null;
      return { 경도: λ0 / RAD, 위도: φ0 / RAD, 배율,
               R: 판.R, R0: 판.R / (배율 || 1), cx: 판.cx, cy: 판.cy,
               rect: canvas.getBoundingClientRect(), 굴림중: !!굴림 };
    },
    /* 그 자리에 «그대로 앉힙니다». 굴리지 않습니다 — 나올 때 나라 지도가
       보여주던 바로 그 그림에서 줌아웃을 시작하려고 씁니다.
       ⚠ rAF 가 아니라 동기 `그리기()` 입니다(숨은 창에서도 돌아야 합니다). */
    앉히기(경도, 위도, 배){
      세우기();
      λ0 = 경도 * RAD;
      φ0 = 기울제한(위도 * RAD);
      배율 = Math.max(1, Math.min(최대배(), 배 || 배율));
      마지막 = 0;
      그리기();
    },
