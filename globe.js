...
/* ── 밤낮(b664, 사용자 요청) ──────────────────────────────────────────
 * 지금 시각으로 태양 직하점을 구해 **밤인 쪽을 어둡게** 합니다.
 * 자료가 하나도 안 듭니다 — 시계만 있으면 됩니다. 그리고 **저절로
 * 바뀝니다**: 새벽에 열면 내 쪽 지구가 그늘입니다.
 *
 * ── 어떻게 그리나 ────────────────────────────────────────────────────
 * 밤과 낮의 경계(터미네이터)는 **태양 방향에 수직인 대원**입니다.
 * 정사도법에서 대원은 «타원 조각»으로 보입니다. 그래서:
 *   ① 태양 벡터 S 에 수직인 단위벡터 둘(u, v)을 잡고
 *      P(t) = u·cos t + v·sin t 로 대원을 한 바퀴 훑습니다.
 *   ② 앞면에 있는 토막(P·e₃ ≥ 0)만 모아 선으로 잇고,
 *   ③ 지구 테두리를 따라 **밤인 쪽으로** 돌아와 닫습니다.
 * ③ 이 핵심입니다 — 곧은 선으로 닫으면 밤이 «렌즈 모양»이 됩니다.
 * 나라를 칠할 때 쓰는 `닫을때돌기` 와 **같은 생각**입니다(위 `그리기고리`).
 *
 * ⚠ 균시차(±16분)와 시차(視差)는 무시합니다. 보여주기용이라 경도로
 *   4° 안쪽이고, 그 정도는 눈에 안 보입니다. 여기에 정밀함을 더할
 *   이유가 없습니다 — 이것은 천문 앱이 아닙니다.
 * ⚠ **연하게.** 이 지구본은 종이 위에 앉은 물건입니다(b585 에 유리
 *   광택을 걷은 것과 같은 이유). 밤이 진해지면 다녀온 나라의 오렌지가
 *   죽습니다 — 그것이 이 화면의 주인공입니다. */
function 밤그리기(ctx, R, cx, cy, λ0, φ0){
  const 지금 = new Date();
  /* 적위 δ — 하지 근처 +23.44°, 동지 근처 -23.44°. */
  const N = (지금 - Date.UTC(지금.getUTCFullYear(), 0, 0)) / 86400000;
  const δ = -23.44 * RAD * Math.cos(2 * Math.PI * (N + 10) / 365.25);
  /* 태양 직하 경도 — UTC 12시에 경도 0, 한 시간에 15°씩 서쪽으로. */
  const utc = 지금.getUTCHours() + 지금.getUTCMinutes() / 60;
  const λs = (12 - utc) * 15 * RAD;
  const S = [Math.cos(δ) * Math.cos(λs), Math.cos(δ) * Math.sin(λs), Math.sin(δ)];

  /* 보는 쪽 밑틀 — globe.js 가 쓰는 투영과 **같은 식**입니다.
     e₁ 동쪽 · e₂ 북쪽 · e₃ 화면 한가운데(=보는 방향). */
  const sλ = Math.sin(λ0), cλ = Math.cos(λ0);
  const sφ = Math.sin(φ0), cφ = Math.cos(φ0);
  const e1 = [-sλ, cλ, 0];
  const e2 = [-sφ * cλ, -sφ * sλ, cφ];
  const e3 = [ cφ * cλ,  cφ * sλ, sφ];
  const 점곱 = (a, b) => a[0]*b[0] + a[1]*b[1] + a[2]*b[2];

  /* 한가운데가 밤인가 — 경계가 안 보일 때 「전부 밤」인지 「전부 낮」인지
     가르는 것이 이것 하나입니다. */
  const 가운데밤 = 점곱(e3, S) < 0;

  /* S 에 수직인 단위벡터 둘. ⚠ S 와 «나란하지 않은» 것에서 시작해야
     외적이 0 이 되지 않습니다 — 그래서 z 축이 S 에 가까우면 x 축을 씁니다. */
  const 씨 = Math.abs(S[2]) < 0.9 ? [0, 0, 1] : [1, 0, 0];
  const 외적 = (a, b) => [a[1]*b[2] - a[2]*b[1], a[2]*b[0] - a[0]*b[2],
                          a[0]*b[1] - a[1]*b[0]];
  const 정규 = a => { const n = Math.hypot(a[0], a[1], a[2]);
                      return [a[0]/n, a[1]/n, a[2]/n]; };
  const u = 정규(외적(씨, S));
  const v = 외적(S, u);

  /* 대원을 한 바퀴. 240 이면 이 크기에서 계단이 안 보입니다. */
  const 칸 = 240, 점 = [];
  for (let i = 0; i < 칸; i++){
    const t = i / 칸 * 2 * Math.PI, ct = Math.cos(t), st = Math.sin(t);
  ...
  /* ⚠ 아주 연하게. 이유는 위 머리말. */
  ctx.fillStyle = 'rgba(38,31,21,.17)';

  /* 앞면으로 «들어오는» 자리를 찾습니다. 없으면 경계가 통째로 뒤에 있는
     것이라 앞면 전체가 낮이거나 전체가 밤입니다. */
  let 시작 = -1;
  for (let i = 0; i < 칸; i++)
    if (점[i].앞 && !점[(i - 1 + 칸) % 칸].앞){ 시작 = i; break; }

  if (시작 < 0){
    if (가운데밤){ ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI * 2); ctx.fill(); }
    ctx.restore();
    return;
  }
  ...
  ctx.beginPath();
  let i = 시작, 끝 = 시작;
  ctx.moveTo(점[i].x, 점[i].y);
  for (let k = 1; k < 칸; k++){
    const j = (시작 + k) % 칸;
    if (!점[j].앞) break;
    ctx.lineTo(점[j].x, 점[j].y);
    끝 = j;
  }
  /* ⚠⚠ **테두리를 «밤인 쪽으로» 돌아 닫습니다.** 곧은 선으로 닫으면
     밤이 렌즈 모양이 되어, 초승달처럼 보여야 할 것이 눈알처럼 보입니다.
     두 방향 중 «가운데 점이 밤인» 쪽을 고릅니다.
   ⚠⚠ **테두리 점의 북쪽 성분은 «+» 입니다(b665 에 고침).**
     투영이 `y = cy − R·(P·e₂)` 이므로, 화면 각 a 인 테두리 점은
         x = cx + R·cos a  →  P·e₁ = cos a
         y = cy − R·sin a  →  P·e₂ = sin a
     즉 **P = cos a·e₁ + sin a·e₂** 입니다. b664 에 `−` 로 적었더니
     「이쪽이 밤인가」가 뒤집혀 **낮인 쪽을 칠했습니다** — 한국이 밤인
     시각(태양 72°W)에 화면 거의 전부가 밝고 오른쪽 위 초승달만
     어두웠습니다. 화면 y 가 아래로 자란다는 것이 이 한 자리에서
     **부호를 뒤집습니다.** 아래 `ctx.arc` 의 `-a0` 도 같은 이유입니다. */
  const 각 = p => Math.atan2(-(p.y - cy), p.x - cx);
  const a0 = 각(점[끝]), a1 = 각(점[시작]);
  const 테두리밤 = a => {
    const P = [Math.cos(a)*e1[0] + Math.sin(a)*e2[0],
  ...
  while (d >  Math.PI) d -= 2 * Math.PI;
  while (d < -Math.PI) d += 2 * Math.PI;
  if (!테두리밤(a0 + d / 2)) d += (d > 0 ? -2 : 2) * Math.PI;
  /* ⚠ 화면 y 는 아래가 «플러스»라, 각도의 도는 쪽이 뒤집힙니다. */
  ctx.arc(cx, cy, R, -a0, -(a0 + d), d > 0);
  ctx.closePath();
  ctx.fill();
  ctx.restore();
...
export function mountGlobe(canvas, 갔다, 처음경도, 처음위도, 누름){
  ...
  const 그리기 = () => {
    ...
    /* ── ⑤½ 밤낮(b664) ── 명암 «아래»에 깔립니다 ────────────────────
       ⚠ 순서가 중요합니다. ⑥ 명암(가장자리 그늘)이 이것 «위»에 와야
         공이 공으로 보입니다 — 밤을 나중에 칠하면 그늘을 덮어서
         지구가 다시 «원판»이 됩니다. */
    밤그리기(ctx, R, cx, cy, λ0, φ0);
