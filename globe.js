...
function 만들기(ctx, R, cx, cy, 고리, λ0, φ0){
  ...
  let 조각 = [];      /* 보이는 토막들 */
  let 지금 = null;
  /* ⚠⚠ **지평선에 «걸렸는지»를 따로 기억합니다(b592).** 토막이 몇 개인지로는
     알 수 없습니다 — 나라가 뒤에서 시작해 앞으로 나왔다 다시 뒤로 넘어가면
     토막은 **하나**인데 잘린 것은 맞습니다. 아래 닫는 자리에서 이 값이
     없으면 그 하나짜리를 «온전한 고리»로 보고 직선으로 닫아 버립니다. */
  let 잘림 = false;
  for (const [λ, φ] of 고리){
    const cφ = Math.cos(φ), sφ = Math.sin(φ);
    const cΔ = Math.cos(λ - λ0);
    const 앞 = sφ0 * sφ + cφ0 * cφ * cΔ;
    if (앞 < 0){ 잘림 = true; if (지금){ 조각.push(지금); 지금 = null; } continue; }
  ...
  /* 마지막 토막에서 첫 토막으로 돌아오는 길도 테두리를 따라갑니다.
     ⚠⚠ **토막이 «하나»여도 잘렸으면 이 길을 따라야 합니다(b592).** ⚠⚠
       전에는 `조각.length > 1` 일 때만 했습니다. 그래서 지평선을 한 번만
       드나든 나라(러시아 등)는 토막이 하나가 되고, 아래 `closePath()` 가
       **끝점에서 시작점까지 직선(현)** 을 그어 지구 위를 세로로 가로지르는
       선이 하나 생겼습니다(사용자 사진).
       예전에는 그 선을 «바다색»으로 그려 안 보였을 뿐입니다 — b585 에서
       테두리를 먹선으로 바꾸자 드러났습니다.
       **안 보이던 것이 색을 바꾸자 드러난 것이지, 새로 생긴 것이 아닙니다.**
       (칠에도 영향이 있었습니다 — 현 안쪽이 잘못 채워지고 있었습니다.) */
  if (조각.length > 1 || 잘림){
