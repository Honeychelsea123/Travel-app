...
import { 가진땅, 나라셀, 도시있나, 상자자르기 } from './citymap.js?v=b711';
import { countryName } from './cities.js?v=b711';
import { visited, myRates } from './rate.js?v=b711';
  ...
  const 그리기 = () => {
    ...
    if (배율 > 도시시작){
      ...
      for (const 나라 of 목록){
         ...
         *   `상자` 는 그 나라 셀이 «절대로 벗어날 수 없는» 테두리라
         *   (citymap.js 에서 그 안에서 잘랐습니다) 이걸로 재면 정확합니다.
         * ⚠ 자료 받기(`가진땅`)를 **이 뒤로** 옮겼습니다 — 화면에 걸린
         *   나라만 50m 를 받습니다. 거르는 이유의 절반이 그것입니다.
         * ⚠ 날짜변경선에 걸친 자리(피지 부근)는 여전히 못 봅니다 —
         *   `창` 도 `상자자르기` 도 이어 붙이지 않은 좌표라 그렇습니다.
         *   고치려면 나라 좌표를 ±1000 만큼 옮겨 한 번 더 그려야 합니다. */
        if (상자[0] > 창[2] || 상자[2] < 창[0] ||
            상자[1] > 창[3] || 상자[3] < 창[1]) continue;
        ...
        for (let i = 0; i < 도시.length; i++){
          ...
          ctx.globalAlpha = 섞 * (갔나 ? 0.62 : 0.06);
