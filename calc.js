 ...
 *
 * ⚠⚠ **핀이 이상한 곳에 꽂힌 것은 좌표가 «없어서»가 아닙니다.** 없으면
 *   `hop` 도 `null` 이고 지도도 그 줄을 건너뜁니다 — 조용합니다.
 *   시끄러운 것은 **틀린 좌표가 박혀 있는 경우**입니다. 도쿄 일정 하나가
 *   273.7km 떨어진 좌표를 달고 「888분 이동」으로 그려졌습니다(외부 QA
 *   2026-09-09). b730 이 **앞으로 들어오는 것**을 막았지만, **이미 박힌
 *   것**은 그대로 그려집니다.
 *
 * ⚠ 판정 기준은 **그 날짜 구간의 도시 중심**입니다. 「일정들의 평균」을
 *   쓰면 틀린 점이 기준을 끌고 갑니다(cands.js 의 `여행기준` 주석).
 * ⚠ 120km 는 b730 의 찾기 반경과 **같은 수**입니다 — 들일 때와 그릴 때가
 *   다른 잣대를 쓰면 「찾을 땐 거절해 놓고 그리기는 그린다」가 됩니다.
 * ⚠ **구간에 도시 중심이 없으면 판정하지 «않습니다».** 잴 기준이 없는데
 *   의심하면 멀쩡한 일정을 지도에서 지웁니다. 모르면 그리는 쪽입니다.
