...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const system = [
      ...
      // ⚠ **세 번째 시도입니다.** JSON 틀 안에 적었더니 두 번 다 무시했습니다 —
      // 준비물을 보고 답해놓고 sources 를 비우거나 "general" 만 적었습니다.
      // 근거 칩은 "이 답이 무엇에 기대고 있는지"를 사용자에게 알리는 자리라,
      // 틀리면 자기 자료로 답한 것을 일반 지식처럼 보이게 합니다.
      // 규칙 자리로 옮기고, 대괄호 이름과 sources 이름을 짝지어 못박습니다.
      '',
      '- **sources 를 반드시 채운다.** 위 자료에서 실제로 읽은 것을 빠짐없이 적는다.',
      '  [일정]→"plans"  [최근 지출]→"expenses"  [구간]→"legs"  [여행]/[오늘]→"trip"',
      '  [일행]→"members"  [예약]→"bookings"  [준비물]→"packing"',
      '  [높게 준 도시]→"ratings"  [취향]→"prefs"',
      '  자료에 없어서 네가 원래 알던 것으로 답한 부분이 있을 때만 "general" 을 더한다.',
      '  예: 준비물 목록을 보고 답했으면 sources 는 ["packing"] 이다. 빈 배열이 아니다.',
      '',
      shots.length
