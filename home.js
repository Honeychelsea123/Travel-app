...
import { $, esc } from './dom.js?v=b702';
import { sb } from './db.js?v=b702';
import { fail, netTimeout, netIsDown, drawOffbar } from './net.js?v=b702';
import { hm, todayYmd } from './calc.js?v=b702';
import { starHtml, paintStars } from './stars.js?v=b702';
/* 평가 히어로는 세 화면이 같은 것을 씁니다 — rateui.js 머리말 참고(b409). */
import { starValue } from './rateui.js?v=b702';
import { cities, countryName, cityCountry } from './cities.js?v=b702';
import { UN_CODES } from './un.js?v=b702';
import { myRates, cityStat, visited } from './rate.js?v=b702';
import { plans } from './trip.js?v=b702';
import { loadCities } from './citysearch.js?v=b702';
/* 지구본에서 나라를 누르면 뜨는 카드가 도시 화면으로 보냅니다(b555). */
import { openCity } from './city.js?v=b702';
import { saveRate, refreshVisited, loadRateData } from './rating.js?v=b702';
/* CONT 는 대륙별 분모(b451) — 지도 화면과 **같은 표**를 씁니다.
   여기서 새로 적으면 두 화면의 분모가 갈라집니다. */
import { openMap, UN_COUNTRIES, CONT, CONT_VIEW, mapBackTo } from './map.js?v=b702';
...
import { drawReport } from './report.js?v=b702';
...
import { PERSONA_BG } from './card.js?v=b702';
/* 성향이 바뀌면 홈 맨 위에 한 번 알립니다(b526) — 「다시 열 이유」. */
import { checkPersonaShift } from './pshift.js?v=b702';
/* 일기장은 제 화면을 엽니다. 「기록 탭에서 왔다」를 적어둬야 닫을 때
   프로필이 아니라 여기로 돌아옵니다(map.js 의 「나온 자리로」와 같은 규칙). */
import { diaryBackTo } from './diary.js?v=b702';
/* 손가락으로 돌려 보는 지구본. **성향 탭에 있던 것을 여기로 옮겼습니다(b542)** —
   이 탭이 곧 「내가 어디를 갔나」입니다. */
import { mountGlobe } from './globe.js?v=b702';
/* 나라 지도 화면(b682). 지구본에서 나라를 누르면 여기로 갑니다. */
import { openCountryMap, setCtryMapCtx } from './ctrymap.js?v=b702';
...
async function 나라눌림(코드){
  if (await openCountryMap(코드)) return;
  return 나라카드(코드);
}
/* 나라 지도가 도시를 누르면 이 카드를 부릅니다. 함수를 넘겨 순환 import
   를 피합니다(앱의 ctx 주입 규약). */
setCtryMapCtx({
  나라카드: (코드, 먼저) => 나라카드(코드, 먼저),
  /* 나라 지도가 화면을 다 덮는 동안 지구본을 세웁니다(b688).
     ⚠ 지구본은 «덮였는지»를 스스로 못 압니다 — 덮는 쪽이 알려 줘야 합니다. */
  지구덮기: (덮였나) => 지구?.덮임?.(덮였나),
  /* 나라를 누르면 그 나라로 굴러가며 확대됩니다(b693). 화면을 닫으면 되돌아갑니다. */
  /* 돌려주는 값은 «얼마나 걸리는가»(초)입니다 — ctrymap 이 그 시간에 맞춰
     판을 띄우고 지구본을 덮습니다(b701). */
  지구다가가기: (경도, 위도, 배) => 지구?.다가가기?.(경도, 위도, 배),
  /* 지구본이 «지금 그리고 있는 자리»를 주고받습니다(b702) — 나라 지도와
     디졸브 없이 이어 붙이는 데 씁니다. ⚠ 안 넘기면 조용히 죽습니다. */
  지구상태: () => 지구?.상태?.() || null,
  지구앉히기: (경도, 위도, 배) => 지구?.앉히기?.(경도, 위도, 배),
  /* 다가가는 나라만 50m 윤곽으로 그립니다(b696). */
  지구자세히: (코드, d) => 지구?.자세히?.(코드, d),
  지구되돌리기: () => 지구?.되돌리기?.(),
});
...
async function renderFoot(통){
  ...
  setTimeout(() => {
    ...
    공 = mountGlobe(공판, gone, undefined, undefined, 나라눌림);
    지구 = 공; 지구갔다 = gone;
