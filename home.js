...
import { $, esc, toast } from './dom.js?v=b744';
import { sb } from './db.js?v=b744';
import { fail, netTimeout, netIsDown, drawOffbar } from './net.js?v=b744';
import { hm, todayYmd } from './calc.js?v=b744';
import { starHtml, paintStars } from './stars.js?v=b744';
/* 평가 히어로는 세 화면이 같은 것을 씁니다 — rateui.js 머리말 참고(b409). */
import { starValue } from './rateui.js?v=b744';
import { cities, countryName, cityCountry } from './cities.js?v=b744';
import { UN_CODES } from './un.js?v=b744';
import { myRates, cityStat, visited } from './rate.js?v=b744';
import { plans } from './trip.js?v=b744';
import { loadCities } from './citysearch.js?v=b744';
/* 지구본에서 나라를 누르면 뜨는 카드가 도시 화면으로 보냅니다(b555). */
import { openCity } from './city.js?v=b744';
import { saveRate, refreshVisited, loadRateData } from './rating.js?v=b744';
/* CONT 는 대륙별 분모(b451) — 지도 화면과 **같은 표**를 씁니다.
   여기서 새로 적으면 두 화면의 분모가 갈라집니다. */
import { openMap, UN_COUNTRIES, CONT, CONT_VIEW, mapBackTo } from './map.js?v=b744';
...
import { drawReport } from './report.js?v=b744';
...
/* ── 지구본이 보던 자리(b715) ─────────────────────────────────────────
 * ⚠ 홈은 별점 하나만 바뀌어도 통째로 다시 그려지고(`lastHomeSig`), 그때
 *   캔버스가 새로 붙어 **지구본이 처음 자리(대한민국·1배)로 돌아갑니다.**
 *   사용자 신고 「뒤로가기하면 제일 초기 화면으로 돌아간다」의 절반이
 *   이것입니다 — 도시 화면을 닫으면 `closeCity` 가 `loadHome()` 을 부릅니다.
 * ⚠ 끝내기 «전»에 붙잡아 둡니다. 캔버스가 사라진 뒤에는 못 묻습니다. */
let 지구자리 = null;
...
/* ── 지구본에서 나라를 눌렀을 때(b682 · b683) ───────────────────────
 * **도시가 하나라도 있으면 나라 지도**를 폅니다(b683, 사용자 결정).
 * ⚠ b682 에는 「셋 이상만」이었습니다 — 영역 방식에서는 도시가 하나여도
 *   그 나라 전체가 그 도시의 땅으로 칠해지므로 볼 것이 있습니다.
 * ⚠ 지도가 아예 없는 나라(투발루)만 카드로 내려갑니다.
 * ⚠ openCountryMap 이 «폈으면 true» 를 돌려줍니다 — 못 폈을 때만 카드로
 *   내려갑니다. 두 길이 «한 자리»에서 갈려야 뒤로가기가 안 꼬입니다. */
/* ── 지구본에서 도시를 누르면(b707) ──────────────────────────────────
 * 사용자 결정: 「나라를 눌러도 반응이 없고, 도시까지 확대된 순간 도시를
 *   누를 수 있으니 그때 도시 카드가 뜨면 되지 않아?」
 * ⚠ 나라 누름은 없어졌습니다 — 갈 나라 페이지가 없습니다.
 * ⚠⚠ **도시 «화면»이 아니라 도시 «카드»입니다(b715, 사용자 요청:
 *   「도시 이름 누르면 바로 도시 페이지로 이동하지 말고 도시 카드가
 *   뜨게 해줘」).** 세 가지가 한꺼번에 낫습니다 —
 *     ① 지구본이 그 자리에 그대로 있습니다. 도시 «화면»으로 가면
 *        돌아올 때 홈이 다시 그려지면서 지구본이 처음 자리로 튑니다
 *        (사용자 신고: 「뒤로가기하면 제일 초기 화면으로 돌아간다」).
 *     ② **별점이 제대로 나옵니다.** 이 카드는 `myRates` 가 비어 있으면
 *        `loadRateData()` 를 한 번 부릅니다 — 평가 탭을 안 열어 본
 *        계정에서 도시 화면이 별을 빈 채로 띄우던 것이 그 때문이었습니다
 *        (사용자 신고: 「별을 남겼던 도시인데도 별점이 안 매겨져 있어」).
 *     ③ 카드에서 **바로 매길 수 있습니다.** 확대해 들어와 그 자리에서
 *        별을 주는 것이 이 지구본이 하려던 일입니다.
 *   더 볼 것이 있으면 카드를 눌러 도시 화면으로 들어갑니다.
 * ⚠ `c.cc` 입니다 — 지구본은 속령을 모국으로 칠하고(괌 → US) 카드도
 *   같은 기준으로 골라야 빈 카드가 안 뜹니다(아래 `나라카드` 주석). */
function 도시눌림(id){
  if (!id) return;
  const c = (cities || []).find(x => x.id === id);
  if (!c) return;
  나라카드(c.cc || c.country, id);
}
...
async function 나라카드(코드, 먼저){
  ...
  if (!Object.keys(myRates || {}).length){
    ...
    /* ⚠ **못 받아도 카드는 띄웁니다(b692).** 별점을 못 받은 것은 「매긴 곳이
       먼저 안 온다」일 뿐이고, 이 카드의 주된 일은 «매기게 하는 것»입니다.
       여기서 죽으면 도시를 눌러도 아무 일이 안 일어납니다. */
    try { await loadRateData(); } catch {}
  ...
  let 목록 = [...매긴, ...유명한것].slice(0, 12);
  /* ⚠ **나라 지도에서 그 도시를 눌러 들어오면 그것이 첫 칸이어야 합니다(b682).**
     위 차례는 「내가 매긴 곳 → 제일 유명한 곳」이고 열두 곳에서 자르므로,
     누른 도시가 목록에 «없을 수도» 있습니다. 앞에 끼워 넣습니다. */
  if (먼저){
    const 그것 = 이나라.find(c => c.id === 먼저);
    if (그것) 목록 = [그것, ...목록.filter(c => c.id !== 먼저)].slice(0, 12);
  }
  if (!목록.length) return;
  const 판 = 시트만들기();
  ...
  판.querySelectorAll('.gsmine .stars').forEach(wrap => {
    wrap.onclick = async e => {
      ...
      const 숫자 = 칸.querySelector('.gsnum'), 이름표 = 칸.querySelector('.gslab');
      /* ⚠⚠ **되돌릴 수 있게 «먼저» 적어 둡니다(b734).** ⚠⚠
         b734 전에는 칠해 놓고 저장 결과를 **버렸습니다.** 실패해도 별은
         칠해진 채 남고 지구본까지 그 나라 색으로 칠해져서, 사용자는
         저장된 줄 알고 넘어갑니다 — 다시 열면 없습니다.
         b716 부터 `saveRate` 가 성패를 돌려주는데 이 자리가 안 보고
         있었습니다(도시 화면은 그때 고쳤습니다).
       ⚠ 먼저 칠하는 것 자체는 그대로 둡니다 — 누른 즉시 반응이 있어야
         합니다. 바뀐 것은 **실패하면 되돌린다**입니다. */
      const 전값 = myRates[wrap.dataset.city]?.stars ?? null;
      paintStars(wrap, v, true);
      숫자.textContent = v ? v.toFixed(1) : '';
      이름표.textContent = v ? '내 별점' : '눌러서 매기기';

      const 됐나 = await saveRate(wrap.dataset.city, { stars: v }, true);
      if (됐나 === false){
        paintStars(wrap, 전값, false);
        숫자.textContent = 전값 != null ? Number(전값).toFixed(1) : '';
        이름표.textContent = 전값 != null ? '내 별점' : '눌러서 매기기';
...
/* ── 지구본 «위»에 뜨는 칩(b704, 사용자 요청) ────────────────────────
 * 사용자: 「다음여행 도쿄까지 5일도 좌측하단에 플로팅으로 띄우는 거 어때?
 *   어차피 여행 없는 사람은 안 보이잖아」 · 「평가를 남기면 지도가 칠해져요
 *   이거도 가입 직후인 사람한테 좌하단 플로팅으로 하자」
 * ⚠ 줄로 세우면 «있는 사람만» 자리를 먹는 게 아니라 **모두의 지구본을
 *   그만큼 눌러 앉힙니다.** 칩으로 띄우면 지구본이 그 자리를 다 씁니다.
 * ⚠ 둘이 겹칠 일은 없습니다 — 여행이 있는 사람은 가입 직후가 아니고,
...
/* ⚠ **통에 담습니다(b706).** 칩이 둘이면 겹칩니다 — 형제 선택자로 두 번째만
 ...
 ⚠⚠ **통은 `body` 바로 밑에 삽니다(b710).** ⚠⚠
   b704부터 b709 까지 에는 `.globebox` 안에 뒀습니다. 그랬더니 **칩의 왼쪽이
   잘렸습니다**(사용자 신고: 「플로팅 좌측 부분이 잘리고」). 자리는
   화면 기준(fixed)으로 제대로 잡히는데 **그리는 것만 탭 판 안쪽으로
   잘렸습니다** — 탭 덱(`#tabdeck`)과 탭 판(`.tabpane`)이 둘 다 스크롤
   칸이고, 웹킷은 스크롤 칸 안의 `position:fixed` 를 그 칸 테두리로
   잘라 냅니다. 나라 카드(`#gsheet`)를 `main` 밖에 둔 것과 같은 이유입니다.
 ⚠ 밖으로 냈으므로 **탭이 숨어도 같이 안 사라집니다.** 대신 app.css 의
   `body:not([data-tab="home"]) .gchips` 가 맡습니다(b709 부터 있던 규칙).
 ⚠ 밖에 사니 홈을 다시 그려도 안 지워집니다 — 그래서 칩마다 «이름표»를
   달고 같은 이름표가 있으면 갈아 끼웁니다. 안 그러면 홈을 다시 그릴
   때마다 「평가를 남기면」이 한 줄씩 쌓입니다. */
...
function 칩놓기(el, 끌때, 이름표){
  el.classList.add('gchip');
  if (이름표) el.dataset.chip = 이름표;
  ...
  let 통 = document.querySelector('body > .gchips');
  if (!통){
    통 = document.createElement('div');
    통.className = 'gchips';
    document.body.appendChild(통);
  }
  /* 같은 이름표가 이미 있으면 갈아 끼웁니다(위 주석). */
  if (이름표) 통.querySelector(`[data-chip="${이름표}"]`)?.remove();
  통.appendChild(el);
}

/* ⚠ **인사·평가재촉은 이제 여기서 안 옮깁니다(b704).** 지구본 «안»에
   들어가 있어서, 여기서 `집.appendChild` 를 하면 도로 끄집어냅니다. */
function 홈정렬(){
  const 집 = $('home');
  if (!집) return;
  const el = $('homefp');
    ...
    return `<div class="rvcity">
      <div class="rrow">
        ...
        <div class="t"><b>${esc(c.name)}</b>
          <span class="memo">${esc(cityCountry(c))}</span></div>
...
async function renderFoot(통){
  ...
  setTimeout(() => {
    ...
    공 = mountGlobe(공판, gone, undefined, undefined, 도시눌림);
    지구 = 공; 지구갔다 = gone;
...
export async function loadFootprint(){
  ...
  sb.from('city_ratings').select('city_id', { count:'exact', head:true })
    ...
    .catch(() => {});
  /* ⚠ `f.rated`(별 준 수)가 아니라 `f.cities`(다녀온 도시 수)입니다
     — 타일 이름이 「다녀온 도시」로 바뀌었습니다(b671, index.html 주석). */
  /* ⚠⚠ **칸이 없으면 여기서 던지고 «아래가 통째로» 안 채워집니다(b697).**
     바로 아래 주석이 그 함정을 경고하는데 정작 이 두 줄에 받침이 없었습니다.
     실제로 재현했습니다 — 기록 탭에서 큰 지도를 열면
     `Cannot set properties of null (setting 'textContent')` 가 나고,
     그 뒤의 깃발·배지·가고 싶은 곳 숫자가 안 채워집니다.
     ⚠ 화면에는 아무 표시도 안 납니다(처리 안 된 거절이라 기록에만 남습니다).
  ...
  { const el = $('s_want'); if (el) el.textContent = f.wants; }
