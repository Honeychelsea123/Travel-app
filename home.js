...
import { fail, netTimeout, netIsDown, drawOffbar } from './net.js?v=b684';
import { hm, todayYmd } from './calc.js?v=b684';
import { starHtml, paintStars } from './stars.js?v=b684';
/* 평가 히어로는 세 화면이 같은 것을 씁니다 — rateui.js 머리말 참고(b409). */
import { starValue } from './rateui.js?v=b684';
import { cities, countryName, cityCountry } from './cities.js?v=b684';
import { UN_CODES } from './un.js?v=b684';
import { myRates, cityStat, visited } from './rate.js?v=b684';
import { plans } from './trip.js?v=b684';
import { loadCities } from './citysearch.js?v=b684';
/* 지구본에서 나라를 누르면 뜨는 카드가 도시 화면으로 보냅니다(b555). */
import { openCity } from './city.js?v=b684';
import { saveRate, refreshVisited, loadRateData } from './rating.js?v=b684';
/* CONT 는 대륙별 분모(b451) — 지도 화면과 **같은 표**를 씁니다.
   여기서 새로 적으면 두 화면의 분모가 갈라집니다. */
import { openMap, UN_COUNTRIES, CONT, CONT_VIEW, mapBackTo } from './map.js?v=b684';
...
import { drawReport } from './report.js?v=b684';
...
import { PERSONA_BG } from './card.js?v=b684';
/* 성향이 바뀌면 홈 맨 위에 한 번 알립니다(b526) — 「다시 열 이유」. */
import { checkPersonaShift } from './pshift.js?v=b684';
/* 일기장은 제 화면을 엽니다. 「기록 탭에서 왔다」를 적어둬야 닫을 때
   프로필이 아니라 여기로 돌아옵니다(map.js 의 「나온 자리로」와 같은 규칙). */
import { diaryBackTo } from './diary.js?v=b684';
/* 손가락으로 돌려 보는 지구본. **성향 탭에 있던 것을 여기로 옮겼습니다(b542)** —
   이 탭이 곧 「내가 어디를 갔나」입니다. */
import { mountGlobe } from './globe.js?v=b684';
/* 나라 지도 화면(b682). 지구본에서 나라를 누르면 여기로 갑니다. */
import { openCountryMap, setCtryMapCtx } from './ctrymap.js?v=b684';
...
/* 나라 지도가 도시를 누르면 이 카드를 부릅니다. 함수를 넘겨 순환 import
   를 피합니다(앱의 ctx 주입 규약). */
setCtryMapCtx({ 나라카드: (코드, 먼저) => 나라카드(코드, 먼저) });
