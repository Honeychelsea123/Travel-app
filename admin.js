...
import { fail, netTimeout } from './net.js?v=b240';
...
export async function loadAdmin(){
  ...
  show(true);
  /* ── 키보드 눈금자를 다시 켭니다 (관리자만, b240) ──
     b178 에서 일부러 껐습니다 — "키보드 문제가 b177 에서 닫혔으므로 관리자
     화면마다 초록 글씨가 뜰 이유가 없다". 맞는 판단이었습니다.
     **그런데 b240 에서 다시 열렸습니다** — 새 여행에서 도시를 검색할 때
     화면이 어긋난다는 신고가 왔는데, 홈 화면 앱은 사파리와 저장 공간이 달라
     `?kb=1` 을 넘길 수가 없어서 **잴 방법이 없습니다.**
     그 자리를 재는 동안만 켜둡니다. 원인을 잡으면 이 줄을 다시 지울 것. */
  window.startRuler?.();
