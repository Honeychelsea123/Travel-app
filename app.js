...
import { $, esc, toast, copyText, md, avatarOf, avatarImg, emptyDo,
         putHtml, dropHtml, toTop, coverDeck } from './dom.js?v=b568';
import { starHtml, paintStars, markRated, armStarDrag } from './stars.js?v=b568';
import { fail, offNote, cacheGet, cacheSet, netIsDown, netTimeout, isOffline,
         write, flushQueue, drawOffbar, setOnDrained,
         setErrLogger, setReadOnly, NOROW } from './net.js?v=b568';
import { loadAdmin } from './admin.js?v=b568';
...
import { recommend, tasteOf, scoreCity, certainPicks } from './rec.js?v=b568';
import { arm, disarm, syncSheets, setSheetCloser, onSwipeX } from './ui.js?v=b568';
...
import { trip, plans, legs, members, expenses, bookings, transitLines,
         ...
         setEditPlanId, setPlanSeedGeo, nameOf } from './trip.js?v=b568';
/* 도시 평가. 네 화면이 같이 쓰는 자료라 한 곳이 어긋나면 넷이 같이 어긋납니다. */
import { myRates, cityStat, visited, justRated, rateFilter, avgTail,
         setRateData, setVisited, applyRate, putCityStat,
         clearJustRated, putRateFilter, clearRates } from './rate.js?v=b568';
/* 도시 사전과 찾기. 한 번 받으면 안 바뀝니다 — 여행이 바뀌어도 사람이 바뀌어도. */
import { cities, countryName, countryInfo, continentOf,
         useCities, addCity, search } from './cities.js?v=b568';
/* 여행 비서가 방금 내놓은 카드. 화면의 번호가 여기를 찾아가므로 통째로 갈아끼웁니다. */
import { suggested, aiTripId,
         setSuggested, clearSuggested, setAiTripId } from './ai.js?v=b568';
/* 성향 카드 화면. app.js 에서 떼어낸 첫 조각입니다(b321) — persona.js 머리말 참고. */
import { openPersona, closePersona, setPersonaCtx,
         personaBackTo } from './persona.js?v=b568';
import { setShiftCtx, clearPcode } from './pshift.js?v=b568';
...
import { openMap, closeMap, openCountries, closeCountries,
         shutBigMap, UN_COUNTRIES, setMapCtx } from './map.js?v=b568';
/* 보관함·배지. app.js 에서 떼어낸 세 번째 조각입니다(b323) — shelf.js 머리말 참고. */
import { openShelf, closeShelf, setShelfCtx } from './shelf.js?v=b568';
/* 일기장(b538) — 도시마다 남긴 일기를 한 장씩 넘겨 봅니다. */
import { openDiary, closeDiary, setDiaryCtx, diaryBackTo } from './diary.js?v=b568';
...
import { openCity, closeCity, setCityCtx,
         isCityOpen, clearCityOpen } from './city.js?v=b568';
...
import { showTyping, hideTyping, growMsg, fitJpeg, drawShot, drawSources,
         aiShots, SHOT_MAX, SRC_KO, setAiUiCtx } from './aiui.js?v=b568';
/* 여행 리포트. app.js 에서 떼어낸 여섯 번째 조각입니다(b333) — report.js 머리말 참고.
   이 화면은 끝에서 다른 화면으로 이어져서 ctx 가 깁니다(함수 다섯). */
import { drawReport, renderAiCard, setReportCtx } from './report.js?v=b568';
...
import { drawCards, openPlanForm, runReview,
         LVCOLOR, setCardsCtx } from './cards.js?v=b568';
import { loadExpenses, setExpenseCtx } from './expense.js?v=b568';
import { loadBookings, loadPacking, loadLinks, closeDocs } from './prep.js?v=b568';
import { loadMembers, handleJoin, ROLE_KO, setMemberCtx } from './member.js?v=b568';
import { drawPlanMap, mapLinks, memoMapUrl, splitParts, ensureLeaflet } from './planmap.js?v=b568';
import { loadCities, drawHits, drawPop, pick, picked, resetPick } from './citysearch.js?v=b568';
import { applyTs, setMyAvatar, setProfileCtx } from './profile.js?v=b568';
import { loadReview, setReviewCtx } from './review.js?v=b568';
import { loadRateData, loadRatings, drawRatings, saveRate, refreshVisited,
         setRateFilter, tripSub, resetRateHtml, setRatingCtx } from './rating.js?v=b568';
import { loadNotifPrefs, loadNotifs, setNotifyCtx } from './notify.js?v=b568';
import { openNew, movePrefs, setNewTripCtx } from './newtrip.js?v=b568';
/* 로그인 전 맛보기 평가(b406). 로그인 화면 안에서만 돕니다 — 앱 전체를
   익명에 열지 않습니다. 자세한 것은 try.js 머리말. */
import { drawTry, claimTryRates } from './try.js?v=b568';
/* 체크 카드로 들어온 사람(b488). 링크 ?check=eu 가 여기로 떨어집니다. */
/* 궁합 링크(?mate=CODE)를 주소에서 받아 담아둡니다 — mate.js 머리말 참고. */
import { catchMate } from './mate.js?v=b568';
/* 연속 평가 — 쭉 매기기(b409). 기록 탭에서 들어갑니다. spree.js 머리말 참고. */
import { openSpree, closeSpree, setSpreeCtx, spreeBackTo } from './spree.js?v=b568';
/* 분석 탭(b439) — 성향·지도로 가는 입구. 화면은 anal.js 가 그립니다. */
import { loadAnal, setAnalCtx } from './anal.js?v=b568';
import { loadHome, loadFootprint, heroTint, tripPhoto, closeReview,
         openTripReport, resetHomeSig, setHomeCtx } from './home.js?v=b568';
import { hhmm, osmLookup, setCandsCtx } from './cands.js?v=b568';
import './selfcheck.js?v=b568';
import { guessCat, setBringCtx } from './bring.js?v=b568';
import { loadTrash, TAB_TRASH, setTrashCtx } from './trash.js?v=b568';
import { review, mins, STAY_MIN, loadAi, setPlanCheckCtx } from './plancheck.js?v=b568';
import { openAi, closeAi, loadChats, aiToBottom, setAiScreenCtx } from './aiscreen.js?v=b568';
import { setAccountCtx } from './account.js?v=b568';
import { openDraft, closeDraft, setDraftCtx } from './draft.js?v=b568';
import { loadTrips, tripFilter, setTripFilter, setTripListCtx } from './triplist.js?v=b568';
import { inTrip, showTab, setTabsCtx } from './tabs.js?v=b568';
import { drawPlans, openPlans, setPlanViewCtx } from './planview.js?v=b568';
import { resetGeo, setGeocodeCtx } from './geocode.js?v=b568';
import { loadLegs, legIn, legFor, fillCityList, setLegsCtx } from './legs.js?v=b568';
import { drawDays, loadPlans, backToList, setTripViewCtx } from './tripview.js?v=b568';
import { drawToday } from './today.js?v=b568';
import { openTrip, fetchTrip, drawTripHeader, unwatch, setOpenTripCtx } from './opentrip.js?v=b568';
import { flags, featOn, loadFlags } from './flags.js?v=b568';
import { setSwRegCtx } from './swreg.js?v=b568';
import { drawCats, parseMemo, nice, lineChips, dayStat,
         catsOpen, setCatsOpen, setPlanLineCtx } from './planline.js?v=b568';
import { distKm, travel, hop, settleMath, dateRange, dayLabel, localTime, money,
         legAt, legNear, legFirst, travelMinutes, NO_CENTS, D1, asDate, hm, ymd, todayYmd } from './calc.js?v=b568';
...
/* ── 문서 자체는 굴러가면 안 됩니다(b567) ─────────────────────────────
 * ⚠⚠ **상단바와 하단바는 `fixed` 이고 본문은 «안쪽 스크롤러»입니다.**
 *   그래서 문서가 조금이라도 굴러가면 본문만 위로 밀려 **바 밑으로
 *   들어갑니다** — 사용자가 겪은 것이 이것입니다: 기록 탭에서 3D/2D 알약이
 *   상단바에 잘리고, 여행 탭에서 여행 머리가 「고정된 것처럼」 본문을
 *   덮었습니다(그 카드는 원래 맨 위에 있을 뿐 고정이 아닙니다).
 * ⚠ 문서가 굴러가는 길은 여럿입니다 — 칸에 커서가 갈 때 브라우저가
 *   옮기거나, 자판이 올라오거나, 뒤로가기 복원이 옛 자리를 되살립니다.
 *   원인을 하나씩 막는 대신 **0 으로 되돌립니다.**
 * ⚠ **탭 화면·여행 화면일 때만입니다.** 도시·후기·서류 화면은 문서가
 *   굴러가는 것이 맞습니다(그쪽은 `window.scrollTo` 로 맨 위를 잡습니다).
 *   거기까지 0 으로 묶으면 긴 글을 못 읽습니다.
 * ⚠ `scrollRestoration` 도 끕니다 — 뒤로가기로 돌아올 때 브라우저가
 *   옛 스크롤 자리를 되살려 같은 일이 벌어집니다. */
try { history.scrollRestoration = 'manual'; } catch {}
addEventListener('scroll', () => {
  const 탭 = !덱.classList.contains('hide');
  const 여행 = !$('tripview').classList.contains('hide');
  if (!탭 && !여행) return;
