...
import { $, esc, toast, copyText, md, avatarOf, avatarImg, emptyDo,
         putHtml, dropHtml, toTop, coverDeck } from './dom.js?v=b750';
import { starHtml, paintStars, markRated, armStarDrag } from './stars.js?v=b750';
import { fail, offNote, cacheGet, cacheSet, netIsDown, netTimeout, isOffline,
         write, flushQueue, drawOffbar, setOnDrained,
         setErrLogger, setReadOnly, NOROW, forgetLocal } from './net.js?v=b750';
import { loadAdmin } from './admin.js?v=b750';
...
import { recommend, tasteOf, scoreCity, certainPicks } from './rec.js?v=b750';
import { arm, disarm, syncSheets, setSheetCloser, onSwipeX } from './ui.js?v=b750';
...
import { trip, plans, legs, members, expenses, bookings, transitLines,
         ...
         setEditPlanId, setPlanSeedGeo, nameOf } from './trip.js?v=b750';
/* 도시 평가. 네 화면이 같이 쓰는 자료라 한 곳이 어긋나면 넷이 같이 어긋납니다. */
import { myRates, cityStat, visited, justRated, avgTail,   /* rateFilter 는 b671 에 뺌 */
         setRateData, setVisited, applyRate, putCityStat,
         clearJustRated, clearRates } from './rate.js?v=b750';
/* 도시 사전과 찾기. 한 번 받으면 안 바뀝니다 — 여행이 바뀌어도 사람이 바뀌어도. */
import { cities, countryName, countryInfo, continentOf,
         useCities, search } from './cities.js?v=b750';   /* addCity 는 b670 에 뺌 */
/* 여행 비서가 방금 내놓은 카드. 화면의 번호가 여기를 찾아가므로 통째로 갈아끼웁니다. */
import { suggested, aiTripId,
         setSuggested, clearSuggested, setAiTripId } from './ai.js?v=b750';
/* 성향 카드 화면. app.js 에서 떼어낸 첫 조각입니다(b321) — persona.js 머리말 참고. */
import { openPersona, closePersona, setPersonaCtx,
         personaBackTo } from './persona.js?v=b750';
import { setShiftCtx, clearPcode } from './pshift.js?v=b750';
...
import { openMap, closeMap, openCountries, closeCountries,
         shutBigMap, UN_COUNTRIES, setMapCtx } from './map.js?v=b750';
/* 보관함·배지. app.js 에서 떼어낸 세 번째 조각입니다(b323) — shelf.js 머리말 참고. */
import { openShelf, closeShelf, setShelfCtx } from './shelf.js?v=b750';
/* 일기장(b538) — 도시마다 남긴 일기를 한 장씩 넘겨 봅니다. */
import { openDiary, closeDiary, setDiaryCtx, diaryBackTo } from './diary.js?v=b750';
...
import { openCity, closeCity, setCityCtx,
         isCityOpen, clearCityOpen } from './city.js?v=b750';
...
import { showTyping, hideTyping, growMsg, fitJpeg, drawShot, drawSources,
         aiShots, SHOT_MAX, SRC_KO, setAiUiCtx } from './aiui.js?v=b750';
/* 여행 리포트. app.js 에서 떼어낸 여섯 번째 조각입니다(b333) — report.js 머리말 참고.
   이 화면은 끝에서 다른 화면으로 이어져서 ctx 가 깁니다(함수 다섯). */
import { drawReport, renderAiCard, setReportCtx } from './report.js?v=b750';
...
import { drawCards, openPlanForm, runReview,
         LVCOLOR, setCardsCtx } from './cards.js?v=b750';
import { loadExpenses, setExpenseCtx } from './expense.js?v=b750';
import { loadBookings, loadPacking, loadLinks, closeDocs } from './prep.js?v=b750';
import { loadMembers, handleJoin, ROLE_KO, setMemberCtx } from './member.js?v=b750';
import { drawPlanMap, mapLinks, memoMapUrl, splitParts, ensureLeaflet } from './planmap.js?v=b750';
import { loadCities, drawHits, drawPop, pick, picked, resetPick } from './citysearch.js?v=b750';
import { applyTs, setMyAvatar, setProfileCtx } from './profile.js?v=b750';
import { loadReview, setReviewCtx } from './review.js?v=b750';
import { loadRateData, loadRatings, drawRatings, saveRate, refreshVisited,
         tripSub, resetRateHtml, setRatingCtx } from './rating.js?v=b750';
import { loadNotifPrefs, loadNotifs, setNotifyCtx } from './notify.js?v=b750';
import { openNew, movePrefs, setNewTripCtx } from './newtrip.js?v=b750';
/* 로그인 전 맛보기 평가(b406). 로그인 화면 안에서만 돕니다 — 앱 전체를
   익명에 열지 않습니다. 자세한 것은 try.js 머리말. */
import { drawTry, claimTryRates } from './try.js?v=b750';
/* 체크 카드로 들어온 사람(b488). 링크 ?check=eu 가 여기로 떨어집니다. */
/* 궁합 링크(?mate=CODE)를 주소에서 받아 담아둡니다 — mate.js 머리말 참고. */
import { catchMate } from './mate.js?v=b750';
/* 연속 평가 — 넘기며 매기기(b409). 기록 탭에서 들어갑니다. spree.js 머리말 참고. */
import { openSpree, closeSpree, setSpreeCtx, spreeBackTo } from './spree.js?v=b750';
/* 분석 탭(b439) — 성향·지도로 가는 입구. 화면은 anal.js 가 그립니다. */
import { loadAnal, setAnalCtx } from './anal.js?v=b750';
import { loadHome, loadFootprint, heroTint, tripPhoto, closeReview,
         openTripReport, resetHomeSig, setHomeCtx } from './home.js?v=b750';
import { hhmm, osmLookup, setCandsCtx } from './cands.js?v=b750';
import './selfcheck.js?v=b750';
import { guessCat, setBringCtx } from './bring.js?v=b750';
import { loadTrash, TAB_TRASH, setTrashCtx } from './trash.js?v=b750';
import { review, mins, STAY_MIN, loadAi, setPlanCheckCtx } from './plancheck.js?v=b750';
import { openAi, closeAi, loadChats, aiToBottom, setAiScreenCtx } from './aiscreen.js?v=b750';
import { setAccountCtx } from './account.js?v=b750';
import { openDraft, closeDraft, setDraftCtx } from './draft.js?v=b750';
import { loadTrips, tripFilter, setTripFilter, setTripListCtx } from './triplist.js?v=b750';
import { inTrip, showTab, setTabsCtx } from './tabs.js?v=b750';
import { drawPlans, openPlans, setPlanViewCtx } from './planview.js?v=b750';
import { resetGeo, setGeocodeCtx } from './geocode.js?v=b750';
import { loadLegs, legIn, legFor, fillCityList, setLegsCtx } from './legs.js?v=b750';
import { drawDays, loadPlans, backToList, setTripViewCtx } from './tripview.js?v=b750';
import { drawToday } from './today.js?v=b750';
import { openTrip, fetchTrip, drawTripHeader, unwatch, setOpenTripCtx } from './opentrip.js?v=b750';
import { flags, featOn, loadFlags } from './flags.js?v=b750';
import { setSwRegCtx } from './swreg.js?v=b750';
import { drawCats, parseMemo, nice, lineChips, dayStat,
         catsOpen, setCatsOpen, setPlanLineCtx } from './planline.js?v=b750';
import { distKm, travel, hop, settleMath, dateRange, dayLabel, localTime, money,
         legAt, legNear, legFirst, travelMinutes, NO_CENTS, D1, asDate, hm, ymd, todayYmd } from './calc.js?v=b750';
...
setShelfCtx({ me: () => me, loadFootprint, openTrip, appTab: () => appTab });
...
/* ⚠ `openTrip` 을 **다시 넣습니다(b631).** b490 에 「읽는 곳이 0」이라
     걷었던 것인데, 오늘의 인사 줄을 누르면 그 여행이 열려야 합니다.
     ⚠ 안 넣으면 기본값(빈 함수)이 조용히 돌아 **아무 일도 안 납니다** —
       오류도 안 납니다. [[ctx-injection-trap]] 그 자체입니다. */
setHomeCtx({ me: () => me, showApp, spreeBackTo, openTrip });
...
$('logout').addEventListener('click', async () => {
  ...
  /* ⚠⚠ **이 기기에 남은 내 자료도 지웁니다(b716, b698 점검 아홉째).**
     못 보낸 저장 대기줄(`t2:queue`)이 그대로 남아 다음 사람에게 흘러갔습니다.
     자세한 사연은 net.js 의 `forgetLocal` 주석에 있습니다. */
  forgetLocal();
...
function 덱으로(t, 부드럽게, 다시){
  ...
  if (i < 0) return;
  /* ⚠⚠ **폭이 0 이면 옮기지 «않고» 다시 옵니다(b729).** ⚠⚠
     `칸폭()` 은 폭이 0 일 때 나눗셈이 NaN 이 되지 않게 **1** 을 돌려줍니다.
     그 1 을 그대로 곱하면 목표가 「3px」 같은 헛값이 되고, 덱은 사실상
     0번 칸에 선 채로 «옮겼다»고 칩니다 — 하단바와 화면이 갈리는 길입니다.
     여행 화면을 닫고 덱을 방금 되살린 참이라 아직 폭이 없을 수 있습니다.
   ⚠ **잠금(`덱잠금`)을 걸기 «전»에 봅니다.** 걸어 놓고 여기서 돌아가면
     푸는 타이머를 못 걸어 **판정이 영영 잠깁니다**(손가락 스와이프로 탭이
     안 바뀝니다).
   ⚠ **스무 번(약 1.2초)까지만.** 끝없이 되물으면 덱이 영영 안 열리는
     자리에서 60ms 짜리 타이머가 영원히 돕니다(globe.js 가 같은 이유로
     마흔 번에서 끊습니다). */
  if (!덱.clientWidth){
    if ((다시 || 0) < 20) setTimeout(() => 덱으로(t, false, (다시 || 0) + 1), 60);
    return;
  }
...
function showApp(t, 표시탭, 이미덱에){
  appTab = t;
  /* ⚠⚠ **지금 어느 탭인지를 body 에 적습니다(b709).** ⚠⚠
     탭 덱은 «가로 스크롤»이라 안 보이는 탭도 `display:none` 이 아닙니다.
     그래서 그 안에 있는 `position:fixed` 짜리는 **스크롤을 벗어나 다섯 탭
     내내 화면에 남습니다** — 기록 탭의 플로팅 칩(b704)이 평가·분석·여행·
     프로필 위에도 떠 있었습니다(실측으로 걸렸습니다).
     ⚠ 판을 숨기는 것으로는 못 고칩니다(덱은 판을 안 숨깁니다).
       「지금 무슨 탭인가」를 CSS 가 알아야 합니다. */
  try { document.body.dataset.tab = t || ''; } catch {}
  shutBigMap();
...
$("appbar").addEventListener("click", e => {
  ...
  /* ⚠⚠ **`appTab` 만 보면 안 됩니다(b729).** ⚠⚠
     `showApp` 은 **맨 첫 줄에서** `appTab = t` 를 박아 놓고 그 뒤에 덱을
     옮깁니다. 그래서 덱 이동이 한 번이라도 실패하면 —
       · `appTab` 은 이미 'rate'
       · 덱은 아직 'anal' 칸
     이 되고, 그 뒤로 평가를 누를 때마다 이 줄이 「같은 탭 재선택」으로 보고
     **맨 위로만 하고 돌아갑니다.** `showApp` 이 다시는 안 불려서 새로고침
     전까지 풀리지 않습니다(외부 QA 리포트 2026-09-09 의 P1 —
     「여러 번 눌러도 그대로, 새로고침 후에만 정상화」).
     ⚠ **`appTab` 은 「가려던 곳」이고 `지금칸()` 은 「지금 서 있는 곳」입니다.**
       재선택인지 아닌지는 뒤엣것으로 물어야 합니다. 둘이 어긋나 있으면
       그것 자체가 「덱을 옮겨야 한다」는 뜻이라, 그냥 showApp 으로 갑니다. */
  const 덱보임 = !덱.classList.contains('hide');
  const 선칸 = 덱보임 ? 탭순서()[지금칸()] : null;
  if (b.dataset.a === appTab && 덱보임 && 선칸 === b.dataset.a){
