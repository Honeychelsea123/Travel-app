...
import { $, esc, toast, copyText, md, avatarOf, avatarImg, emptyDo,
         putHtml, dropHtml, toTop, coverDeck } from './dom.js?v=b504';
import { starHtml, paintStars, markRated, armStarDrag } from './stars.js?v=b504';
import { fail, offNote, cacheGet, cacheSet, netIsDown, netTimeout, isOffline,
         write, flushQueue, drawOffbar, setOnDrained,
         setErrLogger, setReadOnly, NOROW } from './net.js?v=b504';
import { loadAdmin } from './admin.js?v=b504';
...
import { recommend, tasteOf, scoreCity, certainPicks } from './rec.js?v=b504';
import { arm, disarm, syncSheets, setSheetCloser, onSwipeX } from './ui.js?v=b504';
...
import { trip, plans, legs, members, expenses, bookings, transitLines,
         ...
         setEditPlanId, setPlanSeedGeo, nameOf } from './trip.js?v=b504';
/* 도시 평가. 네 화면이 같이 쓰는 자료라 한 곳이 어긋나면 넷이 같이 어긋납니다. */
import { myRates, cityStat, visited, justRated, rateFilter, avgTail,
         setRateData, setVisited, applyRate, putCityStat,
         clearJustRated, putRateFilter, clearRates } from './rate.js?v=b504';
/* 도시 사전과 찾기. 한 번 받으면 안 바뀝니다 — 여행이 바뀌어도 사람이 바뀌어도. */
import { cities, countryName, countryInfo, continentOf,
         useCities, addCity, search } from './cities.js?v=b504';
/* 여행 비서가 방금 내놓은 카드. 화면의 번호가 여기를 찾아가므로 통째로 갈아끼웁니다. */
import { suggested, aiTripId,
         setSuggested, clearSuggested, setAiTripId } from './ai.js?v=b504';
/* 성향 카드 화면. app.js 에서 떼어낸 첫 조각입니다(b321) — persona.js 머리말 참고. */
import { openPersona, closePersona, setPersonaCtx } from './persona.js?v=b504';
...
import { openMap, closeMap, openCountries, closeCountries,
         shutBigMap, UN_COUNTRIES, setMapCtx } from './map.js?v=b504';
/* 보관함·배지. app.js 에서 떼어낸 세 번째 조각입니다(b323) — shelf.js 머리말 참고. */
import { openShelf, closeShelf, setShelfCtx } from './shelf.js?v=b504';
...
import { openCity, closeCity, setCityCtx,
         isCityOpen, clearCityOpen } from './city.js?v=b504';
...
import { showTyping, hideTyping, growMsg, fitJpeg, drawShot, drawSources,
         aiShots, SHOT_MAX, SRC_KO, setAiUiCtx } from './aiui.js?v=b504';
/* 여행 리포트. app.js 에서 떼어낸 여섯 번째 조각입니다(b333) — report.js 머리말 참고.
   이 화면은 끝에서 다른 화면으로 이어져서 ctx 가 깁니다(함수 다섯). */
import { drawReport, renderAiCard, setReportCtx } from './report.js?v=b504';
...
import { drawCards, openPlanForm, runReview,
         LVCOLOR, setCardsCtx } from './cards.js?v=b504';
import { loadExpenses, setExpenseCtx } from './expense.js?v=b504';
import { loadBookings, loadPacking, loadLinks, closeDocs } from './prep.js?v=b504';
import { loadMembers, handleJoin, ROLE_KO, setMemberCtx } from './member.js?v=b504';
import { drawPlanMap, mapLinks, memoMapUrl, splitParts, ensureLeaflet } from './planmap.js?v=b504';
import { loadCities, drawHits, drawPop, pick, picked, resetPick } from './citysearch.js?v=b504';
import { applyTs, setMyAvatar, setProfileCtx } from './profile.js?v=b504';
import { loadReview, setReviewCtx } from './review.js?v=b504';
import { loadRateData, loadRatings, drawRatings, saveRate, refreshVisited,
         setRateFilter, tripSub, resetRateHtml, setRatingCtx } from './rating.js?v=b504';
import { loadNotifPrefs, loadNotifs, setNotifyCtx } from './notify.js?v=b504';
import { openNew, movePrefs, setNewTripCtx } from './newtrip.js?v=b504';
/* 로그인 전 맛보기 평가(b406). 로그인 화면 안에서만 돕니다 — 앱 전체를
   익명에 열지 않습니다. 자세한 것은 try.js 머리말. */
import { drawTry, claimTryRates } from './try.js?v=b504';
/* 체크 카드로 들어온 사람(b488). 링크 ?check=eu 가 여기로 떨어집니다. */
import { drawCheckLanding, checkPicked, clearChecks } from './check.js?v=b504';
/* 궁합 링크(?mate=CODE)를 주소에서 받아 담아둡니다 — mate.js 머리말 참고. */
import { catchMate } from './mate.js?v=b504';
/* 연속 평가 — 쭉 매기기(b409). 기록 탭에서 들어갑니다. spree.js 머리말 참고. */
import { openSpree, closeSpree, setSpreeCtx, spreeBackTo } from './spree.js?v=b504';
/* 분석 탭(b439) — 성향·지도로 가는 입구. 화면은 anal.js 가 그립니다. */
import { loadAnal, setAnalCtx } from './anal.js?v=b504';
import { loadHome, loadFootprint, heroTint, tripPhoto, closeReview,
         openTripReport, resetHomeSig, setHomeCtx } from './home.js?v=b504';
import { hhmm, osmLookup, setCandsCtx } from './cands.js?v=b504';
import './selfcheck.js?v=b504';
import { guessCat, setBringCtx } from './bring.js?v=b504';
import { loadTrash, TAB_TRASH, setTrashCtx } from './trash.js?v=b504';
import { review, mins, STAY_MIN, loadAi, setPlanCheckCtx } from './plancheck.js?v=b504';
import { openAi, closeAi, loadChats, aiToBottom, setAiScreenCtx } from './aiscreen.js?v=b504';
import { setAccountCtx } from './account.js?v=b504';
import { openDraft, closeDraft, setDraftCtx } from './draft.js?v=b504';
import { loadTrips, tripFilter, setTripFilter, setTripListCtx } from './triplist.js?v=b504';
import { inTrip, showTab, setTabsCtx } from './tabs.js?v=b504';
import { drawPlans, openPlans, setPlanViewCtx } from './planview.js?v=b504';
import { resetGeo, setGeocodeCtx } from './geocode.js?v=b504';
import { loadLegs, legIn, legFor, fillCityList, setLegsCtx } from './legs.js?v=b504';
import { drawDays, loadPlans, backToList, setTripViewCtx } from './tripview.js?v=b504';
import { drawToday } from './today.js?v=b504';
import { openTrip, fetchTrip, drawTripHeader, unwatch, setOpenTripCtx } from './opentrip.js?v=b504';
import { flags, featOn, loadFlags } from './flags.js?v=b504';
import { setSwRegCtx } from './swreg.js?v=b504';
import { drawCats, parseMemo, nice, lineChips, dayStat,
         catsOpen, setCatsOpen, setPlanLineCtx } from './planline.js?v=b504';
import { distKm, travel, hop, settleMath, dateRange, dayLabel, localTime, money,
         legAt, legNear, legFirst, travelMinutes, NO_CENTS, D1, asDate, hm, ymd, todayYmd } from './calc.js?v=b504';
...
for (const id of ['personapane', 'shelfpane', 'mappane', 'ctrypane',
                  'setpane', 'admpane']){
  const el = $(id);
  if (!el) continue;
...
function showApp(t, 표시탭, 이미덱에){
  ...
  for (const id of ['ctrypane', 'setpane', 'personapane',
  ...
  if (!이미덱에) 덱으로(t, true);
  덱높이맞추기();
  /* ⚠ **맨 위로 안 보냅니다(b471).** 탭 화면이 각자 스크롤러라 보던 자리가
     저절로 남습니다 — 사용자 결정입니다. 맨 위로 가려면 **같은 탭을 한 번 더**
     누릅니다(아래 appbar). */
}
...
async function render(session){
  ...
  if (!session){
    ...
    /* ── 체크 카드 링크로 왔으면 그것부터(b488) ──────────────────────
       `?check=eu` 로 온 사람은 **자기 24칸**을 보러 온 것입니다. 맛보기
       평가는 「도시 5곳 매기면 성향」이라는 **다른 약속**을 하므로 둘을
       같이 띄우면 무엇을 하라는 건지 안 읽힙니다.
       ⚠ **되돌릴 길을 둡니다.** 도시 자료를 못 받으면 체크 칸을 못
         만드는데, 그때 아무것도 없는 화면이 되면 안 됩니다 —
         drawCheckLanding 이 false 를 주면 맛보기를 그대로 띄웁니다. */
    (async () => { if (!await drawCheckLanding()) drawTry(); })();
  ...
  /* ── 체크한 곳부터 매기기(b488) ─────────────────────────────────────
   * 체크 카드 링크로 들어와 「다녀왔다」고 고른 뒤 로그인한 사람입니다.
   * 화면에서 **「이 n곳부터 별점을 매깁니다」라고 약속**했으므로 지킵니다.
   *
   * ⚠⚠ **체크를 별점 줄로 저장하지 않습니다.** ⚠⚠ 이 앱에서 별점 없는
   *   줄은 「안 가봤어요」입니다(b407). 체크는 「다녀왔다, 별점은 모름」
   *   이라 그 줄로 쓰면 **다녀온 곳이 안 가본 곳으로 남고 다시는 안
   *   물어봅니다.** 그래서 쭉 매기기의 **앞줄**로만 넘깁니다 — 거기서
   *   별을 받아야 비로소 제대로 된 기록이 됩니다.
   * ⚠ **`showApp` 과 `openFromUrl` 뒤**입니다. 먼저 열면 그 위를 홈이
   *   덮습니다(바로 위 openFromUrl 주석과 같은 이유).
   * ⚠ **먼저 비웁니다.** 여기서 실패하든 사람이 바로 나가든, 다음 로그인
   *   때 또 끌려 들어가면 안 됩니다 — 한 번 지킨 약속입니다. */
  const 체크 = checkPicked();
  if (체크.length){ clearChecks(); openSpree(체크); }
}
