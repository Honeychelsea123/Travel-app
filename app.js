...
import { $, esc, toast, copyText, md, avatarOf, avatarImg, emptyDo,
         putHtml, dropHtml, toTop } from './dom.js?v=b472';
import { starHtml, paintStars, markRated } from './stars.js?v=b472';
import { fail, offNote, cacheGet, cacheSet, netIsDown, netTimeout, isOffline,
         write, flushQueue, drawOffbar, setOnDrained,
         setErrLogger, setReadOnly, NOROW } from './net.js?v=b472';
import { loadAdmin } from './admin.js?v=b472';
...
import { recommend, tasteOf, scoreCity, certainPicks } from './rec.js?v=b472';
import { arm, disarm, syncSheets, setSheetCloser, onSwipeX } from './ui.js?v=b472';
...
import { trip, plans, legs, members, expenses, bookings, transitLines,
         ...
         setEditPlanId, setPlanSeedGeo, nameOf } from './trip.js?v=b472';
/* 도시 평가. 네 화면이 같이 쓰는 자료라 한 곳이 어긋나면 넷이 같이 어긋납니다. */
import { myRates, cityStat, visited, justRated, rateFilter, avgTail,
         setRateData, setVisited, applyRate, putCityStat,
         clearJustRated, putRateFilter, clearRates } from './rate.js?v=b472';
/* 도시 사전과 찾기. 한 번 받으면 안 바뀝니다 — 여행이 바뀌어도 사람이 바뀌어도. */
import { cities, countryName, countryInfo, continentOf,
         useCities, addCity, search } from './cities.js?v=b472';
/* 여행 비서가 방금 내놓은 카드. 화면의 번호가 여기를 찾아가므로 통째로 갈아끼웁니다. */
import { suggested, aiTripId,
         setSuggested, clearSuggested, setAiTripId } from './ai.js?v=b472';
/* 성향 카드 화면. app.js 에서 떼어낸 첫 조각입니다(b321) — persona.js 머리말 참고. */
import { openPersona, closePersona, setPersonaCtx } from './persona.js?v=b472';
...
import { openMap, closeMap, openCountries, closeCountries,
         shutBigMap, UN_COUNTRIES, setMapCtx } from './map.js?v=b472';
/* 보관함·배지. app.js 에서 떼어낸 세 번째 조각입니다(b323) — shelf.js 머리말 참고. */
import { openShelf, closeShelf, setShelfCtx } from './shelf.js?v=b472';
...
import { openCity, closeCity, setCityCtx,
         isCityOpen, clearCityOpen } from './city.js?v=b472';
...
import { showTyping, hideTyping, growMsg, fitJpeg, drawShot, drawSources,
         aiShots, SHOT_MAX, SRC_KO, setAiUiCtx } from './aiui.js?v=b472';
/* 여행 리포트. app.js 에서 떼어낸 여섯 번째 조각입니다(b333) — report.js 머리말 참고.
   이 화면은 끝에서 다른 화면으로 이어져서 ctx 가 깁니다(함수 다섯). */
import { drawReport, renderAiCard, setReportCtx } from './report.js?v=b472';
...
import { drawCards, openPlanForm, runReview,
         LVCOLOR, setCardsCtx } from './cards.js?v=b472';
import { loadExpenses, setExpenseCtx } from './expense.js?v=b472';
import { loadBookings, loadPacking, loadLinks, closeDocs } from './prep.js?v=b472';
import { loadMembers, handleJoin, ROLE_KO, setMemberCtx } from './member.js?v=b472';
import { drawPlanMap, mapLinks, memoMapUrl, splitParts, ensureLeaflet } from './planmap.js?v=b472';
import { loadCities, drawHits, drawPop, pick, picked, resetPick } from './citysearch.js?v=b472';
import { applyTs, setMyAvatar, setProfileCtx } from './profile.js?v=b472';
import { loadReview, setReviewCtx } from './review.js?v=b472';
import { loadRateData, loadRatings, drawRatings, saveRate, refreshVisited,
         setRateFilter, tripSub, resetRateHtml, setRatingCtx } from './rating.js?v=b472';
import { loadNotifPrefs, loadNotifs, setNotifyCtx } from './notify.js?v=b472';
import { openNew, movePrefs, setNewTripCtx } from './newtrip.js?v=b472';
/* 로그인 전 맛보기 평가(b406). 로그인 화면 안에서만 돕니다 — 앱 전체를
   익명에 열지 않습니다. 자세한 것은 try.js 머리말. */
import { drawTry, claimTryRates } from './try.js?v=b472';
/* 궁합 링크(?mate=CODE)를 주소에서 받아 담아둡니다 — mate.js 머리말 참고. */
import { catchMate } from './mate.js?v=b472';
/* 연속 평가 — 쭉 매기기(b409). 기록 탭에서 들어갑니다. spree.js 머리말 참고. */
import { openSpree, closeSpree, setSpreeCtx, spreeBackTo } from './spree.js?v=b472';
/* 분석 탭(b439) — 성향·지도로 가는 입구. 화면은 anal.js 가 그립니다. */
import { loadAnal, setAnalCtx } from './anal.js?v=b472';
import { loadHome, loadFootprint, heroTint, tripPhoto, closeReview,
         openTripReport, resetHomeSig, setHomeCtx } from './home.js?v=b472';
import { hhmm, osmLookup, setCandsCtx } from './cands.js?v=b472';
import './selfcheck.js?v=b472';
import { guessCat, setBringCtx } from './bring.js?v=b472';
import { loadTrash, TAB_TRASH, setTrashCtx } from './trash.js?v=b472';
import { review, mins, STAY_MIN, loadAi, setPlanCheckCtx } from './plancheck.js?v=b472';
import { openAi, closeAi, loadChats, aiToBottom, setAiScreenCtx } from './aiscreen.js?v=b472';
import { setAccountCtx } from './account.js?v=b472';
import { openDraft, closeDraft, setDraftCtx } from './draft.js?v=b472';
import { loadTrips, tripFilter, setTripFilter, setTripListCtx } from './triplist.js?v=b472';
import { inTrip, showTab, setTabsCtx } from './tabs.js?v=b472';
import { drawPlans, openPlans, setPlanViewCtx } from './planview.js?v=b472';
import { resetGeo, setGeocodeCtx } from './geocode.js?v=b472';
import { loadLegs, legIn, legFor, fillCityList, setLegsCtx } from './legs.js?v=b472';
import { drawDays, loadPlans, backToList, setTripViewCtx } from './tripview.js?v=b472';
import { drawToday } from './today.js?v=b472';
import { openTrip, fetchTrip, drawTripHeader, unwatch, setOpenTripCtx } from './opentrip.js?v=b472';
import { flags, featOn, loadFlags } from './flags.js?v=b472';
import { setSwRegCtx } from './swreg.js?v=b472';
import { drawCats, parseMemo, nice, lineChips, dayStat,
         catsOpen, setCatsOpen, setPlanLineCtx } from './planline.js?v=b472';
import { distKm, travel, hop, settleMath, dateRange, dayLabel, localTime, money,
         legAt, legNear, legFirst, travelMinutes, NO_CENTS, D1, asDate, hm, ymd, todayYmd } from './calc.js?v=b472';
...
setHomeCtx({ me: () => me, openTrip, showApp, spreeBackTo });
...
function 덱높이맞추기(){
  const el = [...document.querySelectorAll('.tabpane')]
    .find(x => !x.classList.contains('hide'));
  if (!el) return;
  /* 높이를 지운 채로 재야 **원래 자리**가 나옵니다 — 이미 줄여 둔 높이로
     재면 잰 값이 그 높이에 물려 조금씩 밀립니다. */
...
function 밀어넣기(앞탭, t){
  const el = 판찾기(t);
  if (!el || !앞탭 || 앞탭 === t) return;
  if (matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const o = 탭순서();
  const 앞 = o.indexOf(앞탭), 뒤 = o.indexOf(t);
  if (앞 < 0 || 뒤 < 0) return;
  /* 오른쪽 탭으로 가면 오른쪽에서, 왼쪽 탭으로 가면 왼쪽에서 들어옵니다 —
     하단바에서 움직인 방향과 같아야 어디로 갔는지가 읽힙니다. */
  el.classList.remove('slidein-l', 'slidein-r');
  void el.offsetWidth;                       /* 연속으로 눌러도 다시 돌게 */
  el.classList.add(뒤 > 앞 ? 'slidein-r' : 'slidein-l');
  /* ⚠ `animationend` 는 **안 올 수도 있습니다** — 화면이 백그라운드로
     내려가면 애니메이션이 아예 안 돕니다. 클래스가 남으면 다음 전환의
     `remove` 가 치워주긴 하지만, 그 사이 다른 코드가 이 클래스를 보고
     헷갈릴 수 있어 시간으로도 한 번 더 겁니다. */
  const 떼기 = () => el.classList.remove('slidein-l', 'slidein-r');
  el.addEventListener('animationend', 떼기, { once:true });
  setTimeout(떼기, 260);
}
function showApp(t, 표시탭){
  /* 애니메이션 방향을 정하려면 **바꾸기 전 탭**을 알아야 합니다. */
  const 앞탭 = 지난탭; 지난탭 = t;
  appTab = t;
  shutBigMap();
  ...
  $('analview').classList.toggle('hide', t !== 'anal');
  ...
  /* ⚠ **내용을 채우는 load 들보다 뒤**입니다. 앞에 두면 비어 있는 화면이
     밀려 들어왔다가 뒤늦게 채워지는 것이 보입니다. */
  밀어넣기(앞탭, t);
  /* ⚠ **맨 위로 안 보냅니다(b471).** 탭 화면이 각자 스크롤러가 되면서
     보던 자리가 저절로 남습니다 — 사용자 결정입니다. 긴 목록을 보다가
     다른 탭에 다녀오면 그 자리로 돌아옵니다.
     맨 위로 가고 싶으면 **같은 탭을 한 번 더** 누릅니다(아래 appbar). */
  덱높이맞추기();
}
$("appbar").addEventListener("click", e => {
  ...
  /* ⚠ **같은 탭을 다시 누르면 맨 위로(b471).** 보던 자리를 남기기로 했으니
     ...
     긴 목록에서 손가락으로 올리는 것보다 훨씬 빠릅니다. */
  /* ⚠ **같은 탭이어도 그 화면이 닫혀 있으면 엽니다.** `appTab` 은 시작할 때부터
     "home" 이라, 아직 아무 화면도 안 연 상태에서 홈을 누르면 여기 걸려
     아무 일도 안 일어납니다. 재보고 알았습니다. */
  /* ⚠ **보이는 판 아무거나가 아니라 「지금 탭의 판」입니다(b472).**
     `#listview` 에는 `hide` 가 아예 없어서, 아무거나 찾으면 늘 그것이
     잡혔습니다. 그러면 처음 홈을 눌렀을 때 「같은 탭」으로 걸려
     화면이 안 열립니다 — 재보고 알았습니다. */
  const 판 = 판찾기(appTab);
  if (b.dataset.a === appTab && 판 && !판.classList.contains("hide")){
    toTop(판);        /* 같은 탭 다시 누르기 = 맨 위로 */
    return;
  }
  showApp(b.dataset.a);
});
